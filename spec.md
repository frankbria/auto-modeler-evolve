...
- [x] **Class imbalance handling** — When target class distribution is skewed (e.g., 95% no-churn / 5% churn),
      ...
      *Day 41 (20:00): **Chat-triggered imbalance-corrected training** — `_BALANCE_TRAIN_PATTERNS` regex (8 NL variants: "train with class weighting", "apply SMOTE and retrain", "retrain with SMOTE", "use oversampling and train", "fix the imbalance and train", "correct class imbalance and retrain", "train with balanced weights", "use class weighting to train"). `_detect_balance_strategy()` helper extracts "smote"/"threshold"/"class_weight" from the message. Handler fires BEFORE `_TRAIN_PATTERNS` so the strategy isn't lost: launches `_train_in_background()` threads with `imbalance_strategy` param, emits enriched `{type:"training_started", imbalance_strategy}` SSE event. Regression guard: reports N/A clearly. `TrainingStartedResult.imbalance_strategy?` optional TypeScript field; `TrainingStartedCard` updated to show a strategy badge (blue=Class Weighting, violet=SMOTE Oversampling, amber=Threshold Tuning) and mention the strategy in the description text. Closes the user-experience gap: `ClassImbalanceChatCard` tells analysts to "train with class weighting" but previously had no handler — now the full loop is closed conversationally. 26 backend + 5 frontend = 31 new tests. Backend lint: clean. Frontend build: clean.*
...
- [x] **Feature selection automation** — After training, identify features with near-zero importance
      ...
      *Day 42 (04:00): **Chat-triggered retrain excluding weak features** — Closes the conversational gap between `FeatureSelectionCard` (shows weak features) and actual action. `_WEAK_FEAT_RETRAIN_PATTERNS` regex (8 NL variants: "retrain without weak features", "drop weak features and retrain", "remove weak columns and retrain", "exclude low-importance features and train", "train without the weak features", "retrain with weak features removed", "train with unimportant columns excluded", etc.) fires BEFORE `_TRAIN_PATTERNS` + `_BALANCE_TRAIN_PATTERNS` to prevent double-triggering. Handler: guards on `ctx["dataset"]` + completed `ModelRun` with valid model path; loads CSV via `_load_working_df`; calls `identify_weak_features()`; extracts `f["name"]` from weak features list; launches `_train_in_background()` threads excluding those columns; sets `training_started_event` (mutual exclusion); emits `{type:"training_started", excluded_features:[...]}` SSE event. `TrainingStartedResult.excluded_features?: string[]` optional TypeScript field; `TrainingStartedCard` updated to show rose-colored "N feature(s) excluded" badge (`data-testid="excluded-features-badge"`), strikethrough per-feature spans in `excluded-features-list` section, and "without weak features" in description text. Pre-existing `ctx["project"]` → `project` bug fix in `_AUTO_RETRAIN_PATTERNS` and `_FEATURE_SEL_PATTERNS` handlers; `ctx["runs"]` → `ctx["model_runs"]`; `ctx["conversation"]` → `conversation`. 20 backend + 8 frontend = 28 new tests. Backend lint: clean. Frontend build: clean.*
...
- [x] **Fairness / Bias Analysis via chat** — Business analysts can ask "is my model biased?", "check fairness by gender", "any disparate impact?", "statistical parity difference", or "is my model treating everyone fairly?" and receive a `FairnessCheckCard` inline in conversation. `compute_fairness_metrics(y_true, y_pred, sensitive_values, problem_type)` pure function in `core/validator.py`: groups predictions by sensitive column; for classification computes Statistical Parity Difference (SPD = max_rate − min_rate) and Disparate Impact Ratio (DIR = min_rate / max_rate, the "4/5ths rule"); for regression computes per-group MAE and MAE disparity ratio (max/min). Status buckets: `fair` (|SPD|<0.1 AND 0.8≤DIR≤1.2), `warning` (|SPD|<0.2 AND 0.7≤DIR≤1.3), `biased` (otherwise); regression: `fair` (<1.25×), `warning` (<1.5×), `biased` (≥1.5×). Positive label determined globally across all groups to prevent per-group label drift. Zero-MAE edge case treated as disparity=1.0 (fair). Helper labels: `_fairness_spd_label()` (fair/slight/moderate/significant disparity), `_fairness_dir_label()` (passes 4/5ths rule/borderline/fails 4/5ths rule). `GET /api/models/{run_id}/fairness?col=` REST endpoint in `api/validation.py` (400 on unknown column, 400 on high cardinality >50 unique, 404 on unknown run). `_FAIRNESS_PATTERNS` regex (10 NL variants) + `_detect_fairness_col()` longest-match helper in `chat.py`; handler finds best completed run, auto-detects sensitive column from message or falls back to first low-cardinality categorical column, aligns sensitive values with post-dropna rows, calls `compute_fairness_metrics`, emits `{type:"fairness_check"}` SSE event. `FairnessCheckCard` (emerald border=fair, amber=warning, rose=biased, slate=insufficient_data, ⚖️ icon): status badge, algorithm/problem_type/sensitive_col/target_col badges; SPD+DIR 2-column grid (classification); MAE Disparity with plain-English thresholds (regression); per-group metrics table (positive_rate+accuracy for classification, MAE for regression); `role="alert"` callout for warning/biased; summary paragraph; sr-only figcaption. `FairnessCheckResult`/`FairnessGroupMetric` TypeScript interfaces; `fairness_check?` on `ChatMessage`; `attachFairnessCheckToLastMessage` Zustand action; SSE handler + render wired in workspace `page.tsx`.
      *Day 42 (12:00): 44 backend + 26 frontend = 70 new tests. Backend lint: clean. Frontend build: clean.*

- [x] **Per-Class Threshold Tuning via chat** — Business analysts can say "per-class threshold tuning", "optimize threshold for each class", "class-specific thresholds", "independent threshold per class", "multiclass threshold analysis", or 4 other NL variants (8 total in `_PER_CLASS_THRESHOLD_PATTERNS`) and receive a `PerClassThresholdCard` showing the optimal confidence threshold for each class independently via one-vs-rest F1 maximization. Closes the "what confidence cutoff should I use for each class?" gap — distinct from `ThresholdAnalysisCard` (single global threshold for binary/max-confidence proxy) and `ProductionThresholdOptimizerCard` (uses production feedback data). `compute_per_class_threshold_analysis(y_true, y_proba, class_names)` pure function in `core/validator.py`: per-class one-vs-rest sweep (0.05–0.95 thresholds), optimal threshold (max F1), direction (`raise`/`lower`/`keep`), `recommendation` text, `is_actionable` flag (threshold diff ≥10% AND f1_gain >2pp), `n_actionable` count, plain-English summary. `GET /api/models/{run_id}/per-class-threshold` REST endpoint in `api/validation.py` (classification only, predict_proba required, 404 for missing run). `_PER_CLASS_THRESHOLD_PATTERNS` (8 NL variants) + handler in `chat.py`: injects per-class threshold hints into `system_prompt`; emits `{type:"per_class_threshold"}` SSE event. `PerClassThresholdCard` (violet border when actionable / gray when all default, 🎯 icon): n-classes badge, actionable badge, target col badge, summary, per-class rows with direction badge (↑ Raise/↓ Lower/✓ Default OK), F1-gain badge, recommendation text, expandable sweep chart (Recharts LineChart with precision/recall/F1 curves + dashed optimal reference line), how-to-use footer, sr-only figcaption. `PerClassThresholdEntry` + `PerClassThresholdResult` TypeScript interfaces; `per_class_threshold?` on `ChatMessage`; `attachPerClassThresholdToLastMessage` Zustand action; SSE handlers (both EventSource branches) + card render in `project/[id]/page.tsx`.
      *Day 85 (20:00): 33 backend + 24 frontend = 57 new tests. Baseline: 5758 backend / 3267 frontend → **5791 backend / 3291 frontend** (+33 / +24). Backend lint: clean. Frontend build + TypeScript + lint: clean.*
...
- [x] **Deployment versioning and rollback** — When a model is retrained and redeployed, the
      ...
      *Day 66 (20:00): Chat integration wired. `_ROLLBACK_PATTERNS` (8 NL variants: "roll back my model", "rollback to version 2", "revert my deployment", "restore to version 1", "go back to version 3", "show my deployment versions", "deployment version history", "previous version please") + `_ROLLBACK_VERSION_RE` for extracting version number from message. Handler detects list vs rollback intent; list path returns all versions for the deployment; rollback path archives current version and creates a new DeploymentVersion pointing at the target version's model files, performing the rollback inline without calling REST endpoints. SSE emits `{type:"rollback_chat", rollback_chat:{action, deployment_id, current_version_number, rolled_back, rolled_back_to_version, new_version_number, error_message, versions, total_versions}}`. `RollbackChatCard` (indigo border for list, emerald for success, rose for error) in chat: header with version count badge + "Rolled back to vN" badge; error alert with role="alert"; "Endpoint URL unchanged" success note; version history table (v-number, algorithm name, Current/Restored badge, metric, date); footer hint "To roll back, say 'roll back to version N'" in list mode; "Current version is now vN" in rollback mode. `RollbackVersionEntry` + `RollbackChatResult` TypeScript types; `rollback_chat?` on `ChatMessage`; `attachRollbackChatToLastMessage` Zustand action; SSE handler and card render wired in `page.tsx`. 10 backend + 18 frontend = 28 new tests.*
...
- [x] **Deployment Throughput Assessment** — Track D perpetual. Analysts can ask "how long to process 1000 predictions?", "throughput assessment", "how fast can my deployment process requests?", "maximum requests per second", "batch processing time for 500 records", "time to process my dataset", or "how many predictions per second" (8 NL variants in `_THROUGHPUT_PATTERNS`) and receive a `DeploymentThroughputCard` showing latency-based capacity analysis derived from **actual measured `response_ms`** in PredictionLogs. Closes the "can my deployment handle my batch workload?" analyst gap — distinct from `cost_estimate_event` (quota and configured rate-limit based) and `quota_runway` (monthly quota burn). `_format_duration(seconds)` pure helper formats durations human-readably (ms / seconds / minutes / hours). `compute_deployment_throughput(log_dicts, target_n)` pure function in `core/analyzer.py`: filters `response_ms` values (skips None), sorts, computes p50/p95/p99/mean latency from measured values, derives `max_rps = 1000/p95` (conservative single-threaded throughput), `serial_seconds = target_n × p95/1000`, `serial_duration` (formatted), verdicts: `no_data` (<5 samples), `instant` (<5s), `fast` (<60s), `moderate` (<600s), `slow` (<3600s), `very_slow` (≥3600s). Returns all stats + plain-English summary. `GET /api/deploy/{id}/throughput-assessment?n=1000` REST endpoint in `api/deploy.py`: loads last 200 PredictionLogs with response_ms, calls pure function, returns result with deployment_id. `_extract_throughput_n()` helper parses target N from message (k/m suffixes, default 1000). Chat handler guarded by `ctx["deployment"]`; loads 200 logs; injects p95 + max_rps + duration into system_prompt; emits `{type:"throughput_assessment"}` SSE event. `DeploymentThroughputCard` (sky border=fast / amber=moderate / orange=slow / rose=very_slow / emerald=instant / muted=no_data, ⚡ icon): verdict badge, target_n badge, p50/p95/p99/RPS stat boxes, duration estimate row with clock emoji, sample count note, summary paragraph, sr-only figcaption. `DeploymentThroughputResult` TypeScript interface; `throughput_assessment?` on `ChatMessage`; `attachThroughputAssessmentToLastMessage` Zustand action; SSE handlers + card render wired in both EventSource branches of `page.tsx`.
      *Day 85 (12:00): 43 backend + 21 frontend = 64 new tests. Baseline: 5715 backend / 3246 frontend → **5758 backend / 3267 frontend** (+43 / +21). Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Input Feature Drift Ranking by Importance** — Track D perpetual. Analysts can ask "which features drifted the most", "drift ranking by importance", "rank my drifted features", "most important drifting features", "which drift matters most", "feature drift risk", "drift vs importance", or "top risk drift features" (8 NL variants in `_DRIFT_IMPORTANCE_PATTERNS`) and receive a `DriftImportanceCard` that cross-references production OOR/unseen rates with feature importances to surface which drifted features actually matter for prediction quality. Closes the "I know inputs are drifting, but which ones should I care about?" analyst gap — distinct from `CovariateDriftAlertCard` (shows ALL features above drift threshold, no importance weighting). `compute_drift_importance_ranking(all_inputs, feature_ranges, feature_importances, max_features=15)` pure function in `core/analyzer.py`: collects per-feature production values; computes `drift_pct` (OOR% for numeric using min/max, unseen% for categorical); cross-references with normalized `importance_pct`; computes `risk_score = drift_pct × importance_pct`; classifies priority (`critical` = drift≥20% + importance≥median, `high` = drift≥10% or moderate-drift+top-half importance, `medium` = drift≥2%, `low` = any drift, `no_drift`); sorts by risk_score descending; derives verdicts: `action_required`, `attention`, `monitoring`, `clear`, `no_importances`. Returns `ranked_features` list + counts per priority + `n_features` + `n_samples` + `summary`. `GET /api/deploy/{id}/drift-importance-ranking` REST endpoint in `api/deploy.py`: loads 500 PredictionLogs; parses feature_ranges from pipeline; loads model + calls `identify_weak_features()` using `feature_names` from Deployment; calls pure function. Chat handler guarded by `ctx["deployment"]`; loads logs, pipeline, model importances inline; injects verdict + counts into system_prompt; emits `{type:"drift_importance_ranking"}` SSE event. `DriftImportanceCard` (rose=action_required / amber=attention / sky=monitoring / emerald=clear / muted=no_importances, 🎯 icon): verdict badge, feature-count + sample-count badges, priority alert callout for critical/high features (role="alert"), features table with per-row Priority badge + Importance progress bar (indigo) + Drift progress bar (color-coded) + drift details column; no-importances message when model type unsupported; summary paragraph, sr-only figcaption. Full TypeScript wiring: `DriftImportanceFeature`, `DriftImportanceRankingResult` interfaces; `drift_importance_ranking?` on `ChatMessage`; `attachDriftImportanceRankingToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 86 (12:00): 49 backend + 20 frontend = 69 new tests. Baseline: 5816 backend / 3300 frontend → **5865 backend / 3320 frontend** (+49 / +20). Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Deployment Feature Drift Alert Webhook** — Track D perpetual. Analysts can say "enable feature drift alerts", "turn on drift webhook", "alert me when features drift", "notify me on critical feature drift", "feature drift alert", "webhook on feature drift", "proactive drift alerts", "automatic drift notification", "disable drift alert", or "status of feature drift alert" (8 NL variants in `_FEATURE_DRIFT_ALERT_PATTERNS`) to configure proactive webhook delivery when critical-priority features are detected. Closes the "I want to be automatically notified when my important features drift — without polling the dashboard" analyst gap; extends the Day 86 (12:00) drift importance ranking feature with push notifications. `EVENT_FEATURE_DRIFT = "feature_drift"` added to `core/webhook.py` `ALL_EVENTS`. `Deployment` model gets `feature_drift_alert_enabled: bool = False` and `feature_drift_alert_last_fired_at: Optional[datetime] = None`; inline migrations in `db.py`. `_check_and_fire_feature_drift_alert(deployment_id, critical_features, cooldown_hours=24)` pure helper in `api/deploy.py`: opens own DB session; checks `feature_drift_alert_enabled`; applies 24-hour cooldown gate via `feature_drift_alert_last_fired_at`; stamps timestamp before dispatch to prevent racing threads; calls `dispatch_webhooks(EVENT_FEATURE_DRIFT, {critical_feature_count, top_critical_features, message})`; entirely `except Exception: pass` (best-effort). `GET /api/deploy/{id}/drift-importance-ranking` wires background thread: when alert enabled + `ranked_features` has any `action_required`/`attention` priority entries, spawns daemon thread. `PUT /api/deploy/{id}/feature-drift-alert` REST endpoint: enables/disables, resets `last_fired_at` on disable. `GET /api/deploy/{id}/feature-drift-alert-status` REST endpoint. Chat handler: `_DISABLE_FEATURE_DRIFT_ALERT_RE` and `_STATUS_FEATURE_DRIFT_ALERT_RE` sub-patterns for intent disambiguation; handler enables/disables/reads config; emits `{type:"feature_drift_alert_config"}` SSE event. `FeatureDriftAlertCard` (sky border, 🔔 icon): Enabled/Disabled badge, summary text, cooldown info block (with 24-hour strong element), last-fired timestamp, "No alerts fired yet" message when enabled + unfired, footer help text. Full TypeScript wiring: `FeatureDriftAlertConfig` interface; `feature_drift_alert_config?` on `ChatMessage`; `attachFeatureDriftAlertConfigToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 86 (20:00): 21 backend + 16 frontend = 37 new tests. Baseline: 5865 backend / 3320 frontend → **5886 backend / 3336 frontend** (+21 / +16). Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Deployment Segment Drift Detection** — Track D perpetual. Analysts can ask "segment drift analysis", "drift by region", "which segment has the most drift", "geographic drift analysis", "drift breakdown by category", "is drift concentrated in a group", "drift breakdown by segment", or "where is the drift concentrated" (8 NL variants in `_SEGMENT_DRIFT_PATTERNS`) and receive a `SegmentDriftCard` showing which data segments drift most from the training distribution — revealing whether drift is concentrated in a specific region/group or widespread. Closes the "is my model drifting more for one segment than another?" analyst gap — distinct from `DriftImportanceCard` (ranks features, not segments) and `CovariateDriftAlertCard` (binary per-feature threshold). `compute_segment_drift(all_inputs, segment_column, feature_ranges, max_segments=15)` pure function in `core/analyzer.py`: groups PredictionLog inputs by segment column, computes per-segment drift score (average OOR% for numeric + unseen% for categorical across all other features), classifies status (`high`≥20%, `moderate`≥10%, `low`≥2%, `minimal`<2%), returns `ranked_features` per segment (top 3 drifting), `verdict` (`concentrated` when top segment drifts ≥1.5× others, `widespread`, `minimal`, `no_data`), and plain-English `summary`. `GET /api/deploy/{id}/segment-drift?segment_col=region` REST endpoint: loads 300 PredictionLogs, auto-detects the first categorical feature if `segment_col` not specified. `_detect_segment_col(message, feature_ranges)` helper prefers feature names found in the message, falls back to first categorical. Chat handler guarded by `ctx["deployment"]`; emits `{type:"segment_drift"}` SSE event. `SegmentDriftCard` (rose border=widespread / amber=concentrated / emerald=minimal / default=no_data, 🗺️ icon): segment column badge, n-segments badge, verdict badge, summary paragraph, status counts (high/moderate/low/minimal), per-segment rows (name, drift score bar, status badge, top drifting features), avg drift + sample count footer. Full TypeScript wiring: `SegmentDriftEntry`, `SegmentDriftResult` interfaces; `segment_drift?` on `ChatMessage`; `attachSegmentDriftToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 87 (04:00): 35 backend (15 pure-function + 12 regex + 3 REST + 5 helper) + 18 frontend (15 card component + 3 store action) = **53 new tests**. Baseline: 5886 backend / 3336 frontend → **5921 backend / 3354 frontend** (+35 / +18). Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Deployment Prediction Value Trend by Segment** — Track D perpetual. Analysts can ask "prediction trend by segment", "prediction trends per region", "which segment is improving?", "which category is declining?", "segment-level prediction trend", "how are my predictions changing for each region?", "average prediction for West trending", or "which segment has the most improved prediction" (8 NL variants in `_SEGMENT_PRED_TREND_PATTERNS`) and receive a `SegmentPredictionTrendCard` showing whether predicted values are trending up, down, or staying stable for each segment independently. Closes the "is my average predicted revenue trending up or down for the West region?" analyst gap — distinct from `PredictionValueTrendCard` (overall trend, no segment breakdown) and `SegmentDriftCard` (input distribution drift, not output values). `compute_segment_prediction_trend(logs_data, segment_column, problem_type, n_days=30, max_segments=8)` pure function in `core/analyzer.py`: groups enriched prediction log dicts by segment column value extracted from `input_features_dict`; for each segment computes daily mean `prediction_numeric` (regression) or `confidence` (classification), derives `change_pct` (first-to-last period) and `direction` (`trending_up` > +2%/period, `trending_down` < -2%/period, otherwise `stable`); sorts by `|change_pct|` descending so most-moving segments surface first; derives overall `verdict` (`diverging`/`all_improving`/`all_declining`/`mixed`/`stable`/`no_data`); reports `most_improved_segment` and `most_declining_segment`. `GET /api/deploy/{id}/segment-prediction-trend?segment_col=region&n=200&n_days=30` REST endpoint: auto-detects first categorical feature if `segment_col` not specified. `_SEGMENT_PRED_TREND_PATTERNS` (8 NL variants) + `_detect_segment_col()` reused from segment drift for auto-detection + handler in `chat.py` guarded by `ctx["deployment"]`; emits `{type:"segment_pred_trend"}` SSE event. `SegmentPredictionTrendCard` (amber border=diverging / emerald=all_improving / rose=all_declining / sky=mixed / muted=stable, 📈 icon): verdict badge, segment-column badge, n-segments badge, n-days badge, summary paragraph, most-improved/declining callouts with ↑/↓ arrows (emerald/rose), per-segment rows with direction badge (↑ Trending Up / ↓ Trending Down / → Stable), change% label, first/latest/samples grid, per-segment Recharts LineChart sparkline (emerald=up / rose=down / slate=stable), empty state, sr-only figcaption. Full TypeScript wiring: `SegmentPredTrendDirection`, `SegmentPredTrendVerdict`, `SegmentPredTrendDayStat`, `SegmentPredTrendEntry`, `SegmentPredTrendResult` interfaces in `lib/types.ts`; `segment_pred_trend?` on `ChatMessage`; `attachSegmentPredTrendToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 87 (12:00): 42 backend (17 pure-function + 14 regex + 4 REST endpoint + 7 helper) + 19 frontend (16 card component + 3 store action) = **61 new tests**. Baseline: 5921 backend / 3354 frontend → **5963 backend / 3373 frontend** (+42 / +19). Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Deployment Segment Confidence Trend** — Track D perpetual. Analysts can ask "confidence by segment", "model confidence per region", "is my model less confident for West", "confidence trend by segment", "which segment has the lowest confidence", "model uncertainty by region", "confidence breakdown by segment", or "low confidence for each category" (8 NL variants in `_SEGMENT_CONF_TREND_PATTERNS`) and receive a `SegmentConfidenceTrendCard` showing whether the model's confidence is improving, deteriorating, or stable for each segment independently — and whether a calibration gap exists between segments. Closes the "is my model less certain about West region predictions over time?" analyst gap — distinct from `SegmentPredictionTrendCard` (tracks prediction *values*) and `SegmentDriftCard` (tracks input distribution). `compute_segment_confidence_trend(logs_data, segment_column, problem_type, n_days=30, max_segments=8)` pure function in `core/analyzer.py`: for classification tracks average `confidence` score per segment per day (high score = certain predictions); for regression tracks coefficient of variation (`std/|mean| × 100%`) per segment per day as a consistency proxy; direction uses ±2%/period threshold for `deteriorating`/`improving`/`stable`; reports `most_confident_segment`, `least_confident_segment`, `calibration_gap` (True when max−min confidence across segments > 0.15); verdicts: `calibration_gap` / `uniform_decline` / `uniform_improving` / `mixed` / `stable` / `no_confidence_data` / `no_data`. `GET /api/deploy/{id}/segment-confidence-trend?segment_col=category&n=200&n_days=30` REST endpoint: auto-detects first categorical feature if `segment_col` not specified. `_SEGMENT_CONF_TREND_PATTERNS` (8 NL variants) + handler in `chat.py` guarded by `ctx["deployment"]`; reuses `_detect_segment_col()` for auto-detection; emits `{type:"segment_conf_trend"}` SSE event. `SegmentConfidenceTrendCard` (amber border=calibration_gap / rose=uniform_decline / emerald=uniform_improving / sky=mixed / muted=stable, 🎯 icon): verdict badge, segment-column badge, n-segments badge, n-days badge, metric badge (Confidence / Spread CV%), summary paragraph, calibration gap warning block (amber, ⚠), most/least confident callouts with ↑/↓ arrows, per-segment rows with direction badge (↑ Improving / ↓ Deteriorating / → Stable), change% label, first/latest/samples grid, per-segment Recharts LineChart sparkline (emerald=improving / rose=deteriorating / slate=stable), empty state, sr-only figcaption. Full TypeScript wiring: `SegmentConfTrendDirection`, `SegmentConfTrendVerdict`, `SegmentConfTrendDayStat`, `SegmentConfTrendEntry`, `SegmentConfTrendResult` interfaces in `lib/types.ts`; `segment_conf_trend?` on `ChatMessage`; `attachSegmentConfTrendToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 87 (20:00): 47 backend (25 pure-function + 14 regex + 4 REST endpoint + 4 helper) + 20 frontend (20 card component) = **67 new tests**. Baseline: 5963 backend / 3373 frontend → **6010 backend / 3394 frontend** (+47 / +21). Backend lint: clean. Frontend build + TypeScript + lint: clean.*
...
- [x] **Prediction Error Distribution via Chat** — Track D perpetual. Analysts can ask "show me the error distribution", "residual histogram", "how are my errors distributed?", "where does my model struggle?", "per class error rate", and 7 other NL variants to see a histogram of ALL prediction errors (not just the top-N worst cases like `PredictionErrorCard`). Pure function `compute_error_distribution(y_true, y_pred, problem_type, n_bins, target_classes)` in `core/validator.py`: regression path bins residuals (actual − predicted) into 5–30 histogram bars with lo/hi/count/pct/label per bin; stats include mean/std/MAE/bias_label/bias_pct/within_1std_pct/total; bias detection normalizes mean residual to data range (unbiased <5%, over/under-predicts otherwise). Classification path returns per-class error rates sorted highest-to-lowest with class/total/wrong/error_rate/error_pct; overall stats include total/total_wrong/overall_error_rate/overall_accuracy/n_classes; decodes numeric class indices via `target_classes` when available. Both paths return plain-English `summary`. `GET /api/models/{run_id}/error-distribution?n_bins=10` in `api/validation.py` (shared `_load_run_context()` + `_build_Xy()` helpers; adds algorithm/target_col; 404 for unknown run). `_ERROR_DIST_PATTERNS` (11 NL variants; guarded by `not pred_error_event` to prevent collision) in `chat.py`; SSE type `error_distribution`. `ErrorDistributionCard` (indigo border, 📊 icon): regression shows color-coded Recharts BarChart (rose=negative residuals, orange=positive, emerald=near-zero), stats grid (mean/std/MAE/within-1std), `BiasLabel` badge (emerald=unbiased, amber=under-predicts, rose=over-predicts), axis labels, aria-label; classification shows stats grid (accuracy/total-wrong/classes), sortable per-class table with proportional error bars color-coded rose/amber/emerald, "Sorted highest error rate first" note. `ErrorDistributionBin` + `ErrorDistributionClassRow` + `ErrorDistributionStats` + `ErrorDistributionResult` TypeScript interfaces; `error_distribution?` on `ChatMessage`; `attachErrorDistributionToLastMessage()` Zustand store action.
      *Day 65 (04:00): 34 backend (10 regression unit + 8 classification unit + 3 integration + 13 chat-pattern) + 20 frontend = 54 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Model Card Export via Chat** — Track D perpetual. Analysts can ask "export model card", "download model card", "model card for compliance", "generate a model card", "share model documentation", and 6 other NL variants to receive a `ModelCardExportCard` in chat with a download button for a self-contained HTML model card document. `generate_model_card_html()` pure function in `core/report_generator.py` produces a standards-aligned HTML model card: header with project name + badges, overview table (algorithm, target, problem type, rows, features), performance section (primary metric + plain-English interpretation + optional summary), top-8 feature importance bars, optional calibration section (Brier score, calibration note), intended use + limitations, deployment section (endpoint URL when deployed), footer (generation timestamp). `html_escape()` helper ensures XSS-safe rendering of all user-supplied strings. `GET /api/models/{run_id}/export-model-card` endpoint in `api/models.py` loads the run, project, feature set, and dataset; computes feature importances from joblib; checks deployment status; calls `generate_model_card_html()`; returns `HTMLResponse` with `Content-Disposition: attachment` header. `_MODEL_CARD_EXPORT_PATTERNS` regex (10 NL variants) in `chat.py`; handler finds selected/best completed run, builds SSE payload (run_id, project_name, algorithm, algorithm_plain, problem_type, target_column, metric_name, metric_value, metric_display, feature_count, row_count, trained_at, download_url, summary), emits `{type:"model_card_export"}` SSE event. `ModelCardExportInfo` TypeScript interface; `model_card_export?` on `ChatMessage`; `attachModelCardExportToLastMessage` Zustand action; `api.models.exportModelCardUrl(runId)` client helper; `ModelCardExportCard` (indigo border, 📋 icon): algorithm/problem-type/target badges, metric row with F1/R² + feature count + row count, training date, "Download HTML Model Card" anchor button. SSE handler + card render wired in workspace `page.tsx`. Closes the compliance/governance sharing gap — analysts can produce a shareable model accountability document in one conversational step.
      *Day 65 (12:00): 25 backend (10 HTML unit + 3 endpoint integration + 12 regex) + 13 frontend = 38 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Calibration Inline in Training Panel** — Track C perpetual. After each classification training run, the `RunCard` in `ModelTrainingPanel` now renders a `CalibrationRow` beneath `CvScoreRow`. The row reads `metrics.brier_score` (already computed at training time by `_add_calibration_metrics()`) and displays: 🎯 icon, "Brier score: 0.XX", quality label (excellent < 0.1, good < 0.1–0.2, poor ≥ 0.2) color-coded emerald/amber/rose, and a tooltip with the full `calibration_note`. Regression models return null (no Brier score). `data-testid="calibration-row"` for testing. Gives analysts the calibration quality signal directly in the training panel without needing to ask via chat — closes the "how trustworthy are the confidence scores?" gap that previously required a separate chat command.
      *Day 65 (12:00): No new tests (uses existing calibration data from training; covered by build verification).*
- [x] **Automated Model Comparison Summary via Chat** — Track B perpetual. Analysts can ask "compare my models", "model comparison summary", "how do my trained models stack up", "model showdown", "which model is best?", "model overview", "summarize my models", or 10 other NL variants and receive a `ModelComparisonSummaryCard` in chat with narrative comparison of all completed model runs — no criteria required (distinct from `ModelSelectionCard` which requires explicit constraints). `compute_model_comparison_summary(runs)` pure function in `core/advisor.py`: ranks all runs by primary metric (R² for regression, accuracy for classification), builds per-run display dicts via `_build_run_summary()` (algorithm_plain from `_ALGO_PLAIN`, explainability label from `_EXPLAINABILITY_LABEL`, speed label from `_SPEED_LABEL`, CV mean/std when available, is_selected/is_deployed flags), generates up to 3 plain-English trade-off sentences (accuracy gap, explainability trade-off when winner ≠ most interpretable, stability from CV std), builds 2–3 sentence narrative (single-run hint for only-run case, runner-up mention for multi-run), returns structured dict with n_runs/winner/runs_compared/trade_offs/narrative/summary/problem_type/only_one_run. `_MODEL_COMPARISON_SUMMARY_PATTERNS` regex (15 NL variants; no trailing `\b` per CLAUDE.md rule) in `chat.py`; handler fires when pattern matches AND `_MODEL_SELECT_PATTERNS` does NOT match; loads all `done` model runs, serializes them to plain dicts, calls `compute_model_comparison_summary()`, injects narrative into system prompt; emits `{type:"model_comparison_summary"}` SSE event. `ModelComparisonRunSummary` + `ModelComparisonSummaryResult` TypeScript interfaces; `model_comparison_summary?` on `ChatMessage`; `attachModelComparisonSummaryToLastMessage()` Zustand store action; `ModelComparisonSummaryCard` (blue border, 📊 icon): header with run count + problem type + metric badge, narrative paragraph, comparison table (Algorithm | Metric | CV 5-fold | Interpretability | Speed) with winner row highlighted + ✓ badge and (selected)/(live) annotations, trade-off bullet list, italic summary footer, sr-only figcaption for accessibility. 23 backend + 15 frontend = 38 new tests.
      *Day 65 (20:00): 23 backend + 15 frontend = 38 new tests. Total: 4131 backend + 2256 frontend = 6387.*
- [x] **Cross-Model Feature Importance Comparison via Chat** — Track B perpetual. Analysts can ask "which features matter most across all my models?", "feature importance comparison", "which variables are important?", "top features", "what drives predictions?", "feature consensus", or 8 other NL variants and receive a `CrossModelFeaturesCard` in chat. `compute_cross_model_feature_importance(runs_with_importances)` pure function in `core/advisor.py` accepts `[{run_id, algorithm, algorithm_plain, importances:[{feature,importance,rank}]}]`; aggregates per-feature mean_importance, consistency (CoV < 0.3 = "high", 0.3–0.7 = "medium", > 0.7 = "variable"), agreement_count (models where rank ≤ 5); caps at 15 features; identifies consensus_features (rank ≤ 5 in ALL models). `_build_cross_model_summary()` produces a plain-English takeaway — leads with consensus features when they exist, otherwise lists top-3 by mean importance. `_CROSS_MODEL_FEAT_PATTERNS` regex (13 NL variants). Chat handler loads CSV, applies transformations, loads each done run's joblib model, calls `compute_feature_importance()`, calls pure function, injects summary into system prompt, emits `{type:"cross_model_features"}` SSE event. `GET /api/models/{project_id}/cross-model-features` REST endpoint. `CrossModelFeaturePerModel`/`CrossModelFeatureEntry`/`CrossModelFeatureResult` TypeScript interfaces; `cross_model_features?` on `ChatMessage`; `attachCrossModelFeaturesToLastMessage()` Zustand action; `api.models.crossModelFeatures(projectId)` client method; `CrossModelFeaturesCard` (violet border, 🔍 icon): header, summary, consensus chip callout, feature table (rank | Feature name | mean importance bar | top-5 agreement count | consistency badge), legend footer; SSE handler + render wired in `page.tsx`. 19 backend + 15 frontend = 34 new tests.
      *Day 66 (04:00): 19 backend + 15 frontend = 34 new tests.*

- [x] **Model Accuracy Degradation Alert via Chat** — Track D perpetual. Analysts configure accuracy-based webhook alerts entirely through chat: "alert me when my model accuracy drops below 80%", "set accuracy alert at 0.75", "configure accuracy degradation alert", "disable accuracy alert", "show accuracy alert threshold", or 8 other NL variants. `_compute_feedback_accuracy_simple(session, deployment)` computes live metric from `FeedbackRecord` table — classification: accuracy (0–1), regression: pct_error (0–100). `_check_and_fire_accuracy_alert(deployment_id, problem_type, metric_value, threshold)` fires `EVENT_ACCURACY_ALERT` webhook when metric crosses threshold. `accuracy_alert_fired` flag prevents repeated alerts; resets when threshold changes. `PUT /api/deploy/{id}/accuracy-alert` (set/clear threshold, validates range), `GET /api/deploy/{id}/accuracy-alert-status` (live metric + config). `Deployment` model gains `accuracy_alert_threshold: Optional[float]` + `accuracy_alert_fired: bool`; inline migration adds both columns. Alert wired into `submit_feedback()` after each feedback commit. `AccuracyAlertConfig` TypeScript interface; `accuracy_alert_config?` on `ChatMessage`; `attachAccuracyAlertConfigToLastMessage()` Zustand action; `api.deploy.accuracyAlertStatus()` + `api.deploy.setAccuracyAlert()` client methods; `AccuracyAlertCard` (amber border, 🎯 icon): enabled/disabled badge, fired badge, threshold display, current metric vs threshold, breach color coding, n_feedback, help footer; SSE handler + render wired in `page.tsx`. 21 backend + 20 frontend = 41 new tests.
      *Day 66 (12:00): 21 backend + 20 frontend = 41 new tests. Backend lint: clean. Frontend build: clean.*

- [x] **Interactive What-if Scenario Explorer with Sliders** — Track C perpetual. Upgraded the basic text-input `WhatIfCard` in `DeploymentPanel` into a real-time interactive slider panel. Analysts drag sliders for numeric features (bounded by p5/p95 training-data percentiles, fallback to min/max, then 0–100) and choose from dropdowns for categorical features; the predicted outcome updates in ~400ms (debounced `api.deploy.predict()` call). Side-by-side "Baseline (means)" vs "Your Scenario" comparison with delta badge (▲/▼/→ + % change). Confidence interval band shown when available. "Show more" toggle collapses to 8 features by default. Backend: `get_feature_schema()` in `core/deployer.py` extended to include `min`, `max`, `p5`, `p95` from stored `feature_ranges` dict (`PredictionPipeline` already stores this at deploy time). `FeatureSchemaEntry` TypeScript type extended with optional range fields (`min`, `max`, `p5`, `p95`). Helper functions `fmtNum()` + `buildDefaults()` (numeric: mean → median → 0; categorical: first option). `data-testid="interactive-whatif-card"` on card root. 7 backend + 17 frontend = 24 new tests.
      *Day 67 (04:00): 7 backend + 17 frontend = 24 new tests. Total: 4188 backend + 2326 frontend = 6514. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Prediction Confidence Thresholding via Chat** — Track D perpetual. Analysts can say "set confidence threshold to 80%" or "reject low-confidence predictions" and have uncertain predictions automatically flagged in the API response rather than silently served. `_CONFIDENCE_THRESHOLD_PATTERNS` (9 NL variants: set/configure/enable confidence threshold, minimum confidence filter, only accept above X% confidence, reject low-confidence predictions, confidence filter/cutoff/gate/guard/minimum, disable/remove/clear confidence threshold, check/show confidence threshold) + `_CONFIDENCE_THRESHOLD_VALUE_RE` (normalize "80%" → 0.8) + `_DISABLE_CONFIDENCE_THRESHOLD_RE` in `chat.py`. Handler detects enable/disable/status intent, validates 0.0–1.0 range, persists `confidence_threshold` to `Deployment.confidence_threshold` (nullable float), counts recent below-threshold predictions, emits `{type:"confidence_threshold_config"}` SSE event. `make_prediction()` in `deploy.py`: when threshold is set and `problem_type == "classification"`, compares `max(predict_proba)` (the `confidence` field) against the threshold; if below → adds `below_threshold=True` + `threshold_message` (plain-English: "Model confidence X% is below the minimum threshold of Y%. This prediction may be unreliable — consider providing more specific feature values."); if above → `below_threshold=False`. `PUT /api/deploy/{id}/confidence-threshold` (set/clear, validates 0–1) + `GET /api/deploy/{id}/confidence-threshold-status` (threshold, enabled flag, below_count_30d, total_30d, below_pct, summary) REST endpoints. `ConfidenceThresholdCard` (amber border, 🎯 icon) in chat: enabled/disabled badge with threshold %, amber explanation paragraph when enabled, 30-day below-count stats with %, help text footer. `predict/[id]/page.tsx` shows amber "Low-confidence prediction" callout when `below_threshold=True`. `ConfidenceThresholdConfig` TypeScript type; `attachConfidenceThresholdConfigToLastMessage` Zustand action; `api.deploy.setConfidenceThreshold()` + `api.deploy.getConfidenceThresholdStatus()` client methods. Inline migration for `confidence_threshold REAL` column in `db.py`. Classification-only (regression has no predict_proba; regression models ignore the threshold). Closes the "silent uncertain prediction" gap — a smart colleague would say "I'm not sure about this one" rather than confidently serving a 51% probability answer.
      *Day 67 (12:00): 17 backend + 15 frontend = 32 new tests. Total: 4205 backend + 2341 frontend = 6546. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Prediction Input Validation Rules via Chat** — Track D perpetual. Analysts can say "validate that units is between 1 and 10000", "require region to be one of East, West, Central", "ensure customer_id is not null", "show my validation rules", or "remove all validation rules" and have the deployed prediction API enforce business-rule constraints on incoming inputs. `InputValidationRule` SQLModel table: id (UUID PK), deployment_id (indexed FK), feature_name, rule_type ("range"|"one_of"|"not_null"), min_val (float|None), max_val (float|None), allowed_values (JSON-encoded list|None), created_at. `validate_prediction_inputs(inputs, rules)` pure function in `core/validator.py`: range check (bounds-inclusive, non-numeric rejection), one_of check (JSON-decoded allowed list, case-insensitive string comparison), not_null check; returns `(is_valid, violations)` where each violation carries feature_name/value/rule_type/message. `make_prediction()` in `deploy.py` hooks in after monthly quota: loads rules for the deployment, calls pure function, raises HTTP 422 with joined plain-English violation messages when invalid. REST endpoints: `POST /api/deploy/{id}/input-validation-rules` (create, validates rule_type + required params), `GET /api/deploy/{id}/input-validation-rules` (list all), `DELETE /api/deploy/{id}/input-validation-rules/{rule_id}` (delete one). `_INPUT_VALIDATION_PATTERNS` + `_IV_LIST_RE` + `_IV_DELETE_RE` + `_IV_RANGE_RE` + `_IV_BOUND_RE` + `_IV_ONE_OF_RE` + `_IV_NOT_NULL_RE` regex constants in `chat.py`; handler detects LIST/DELETE/CREATE intent, parses rule from NL message, persists to DB, emits `{type:"input_validation_rule"}` SSE event. `InputValidationRuleEntry` + `InputValidationRuleResult` TypeScript interfaces; `input_validation_rule?` on `ChatMessage`; `attachInputValidationRuleToLastMessage` Zustand store action; `api.deploy.listInputValidationRules()` + `createInputValidationRule()` + `deleteInputValidationRule()` client methods; `InputValidationRuleCard` (violet border/🛡️ for created, slate/📋 for list, rose/🗑️ for deleted, slate/💡 for guidance): `RuleTypeBadge` (Range/One of/Required), `RuleRow` with `data-testid="validation-rule-row-{feature_name}"`, action-specific badges and footer hints. SSE handler + card render wired in workspace `page.tsx`. Closes the "garbage-in prediction" gap — analysts can now protect their prediction APIs from nonsensical inputs without writing any code.
...
- [x] **Prediction Dashboard Field Configuration via Chat** — Track D perpetual. Analysts can say "hide units from the dashboard", "lock region to North", "only show units and revenue", "show all fields", or "what's visible on my dashboard" to control which fields appear on the shared VP-facing prediction URL. `DashboardFieldConfig` SQLModel table: id (UUID PK), deployment_id (indexed), feature_name, is_visible (bool, default True), is_locked (bool, default False), locked_value (str|None — JSON-encoded value when locked), display_label (str|None), display_order (int|None), created_at. REST endpoints: `GET /api/deploy/{id}/dashboard-config` (merged feature schema + config; returns visible_count/locked_count/total_count + per-field state), `PUT /api/deploy/{id}/dashboard-config` (batch upsert via DashboardConfigBatchBody), `DELETE /api/deploy/{id}/dashboard-config` (reset, removes all config records). `_DASHBOARD_CONFIG_PATTERNS` (6 NL arms) + `_DC_HIDE_RE` / `_DC_LOCK_RE` / `_DC_ONLY_SHOW_RE` / `_DC_RESET_RE` / `_DC_STATUS_RE` + `_extract_dashboard_feature()` helper in `chat.py`. Handler detects reset/status/only-show/hide/lock intent, applies changes inline, emits `{type:"dashboard_config"}` SSE event. `predict/[id]/page.tsx` loads dashboard config via `useEffect`, filters hidden fields from form schema, shows locked fields as read-only inputs pre-filled with locked_value, includes locked values in prediction payload, shows "Simplified view: N fields hidden, N fields locked" notice in card header. `DashboardFieldChange` + `DashboardConfigResult` TypeScript interfaces; `dashboard_config?` on `ChatMessage`; `attachDashboardConfigToLastMessage` Zustand action; `api.deploy.getDashboardConfig()` + `updateDashboardConfig()` + `resetDashboardConfig()` client methods; `DashboardConfigCard` (emerald border for updated, sky for status, slate for reset): `FieldRow` with Hidden/Locked/Visible badges, data-testid attrs. SSE handler + card render wired in workspace `page.tsx`. Closes the "VP confusion" gap — analysts can now curate which fields their VP sees, pre-lock context fields like region, and hide internal identifiers without touching any code.
...
- [x] **Per-field display labels via Chat** — Track D perpetual. Analysts say "label units as 'Monthly Units Sold'" or "rename region as Sales Region on the dashboard" to set user-friendly display names. `_DC_LABEL_RE` regex + "labeled" arm in `_DASHBOARD_CONFIG_PATTERNS`. Handler upserts `DashboardFieldConfig.display_label`, emits `action="labeled"` SSE event with `labeled_count`. `DashboardFieldChange.display_label?: string`, `DashboardConfigResult.action` union extended + `labeled_count?`. `DashboardConfigCard`: violet badge `→ "label"` in FieldRow; "Field Labeled" heading/border/icon for labeled action; `labeled_count` badge in header. `predict/[id]/page.tsx` already consumed `display_label` (Day 68 04:00). End-to-end: analyst curates VP-facing label names without any code.
      *Day 68 (20:00): 9 backend + 6 frontend = 15 new tests. Total: 4279 backend + 2397 frontend = 6676. Backend lint: clean. Frontend build: clean.*
...
- [x] **Prediction form "Copy as link"** — Track D perpetual. VPs can bookmark or share exact prediction scenarios as pre-filled URLs. `ShareLinkCard` (orange border, 🔗 icon) in chat; `GET /api/deploy/{id}/share-link?features={"units":"100"}` REST endpoint; `_SHARE_LINK_PATTERNS` + `_SHARE_LINK_VALUE_RE` regexes (9 NL variants); URL param parsing on form mount; copy-to-clipboard on the form card itself.
      *Day 70 (04:00): 24 backend + 18 frontend = 42 new tests. Total: 4356 backend + 2454 frontend = 6810. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Weekly Deployment Usage Report via Chat** — Track D perpetual. Analysts ask "how many predictions did I get this week?", "weekly prediction summary", "weekly report", "how did I do this week" (8 NL variants) and receive a `WeeklyUsageReportCard` with this-week vs last-week count, trend (↑/↓/→), 7-day breakdown bar chart, and top input patterns table. `_WEEKLY_USAGE_PATTERNS` regex + handler in `chat.py`; `WeeklyUsageReportResult` TypeScript type; `attachWeeklyUsageReportToLastMessage` Zustand action; SSE handler + render wired in workspace `page.tsx`.
      *Day 70 (12:00): 21 backend + 16 frontend = 37 new tests. Total: 4356 backend + 2454 frontend = 6810. Backend lint: clean. Frontend build: clean.*

- [x] **Cross-Project Model Comparison via Chat** — Track B perpetual. Analysts ask "cross-project comparison", "compare my revenue model vs my churn model", "rank all my models side by side", "which of my models performs best overall", "how do all my models compare across projects" (8 NL variants) and receive a `CrossProjectComparisonCard` with head-to-head normalized 0-100 performance scores across all projects. Normalization: R²/accuracy/F1/precision/recall/AUC → ×100; error metrics (MAE/MSE/RMSE/MAPE) → 1/(1+x)×100; unknown metric in 0–1 range → ×100; else 50. `_normalize_metric()` pure helper + `compute_cross_project_comparison(project_summaries)` pure function in `core/advisor.py` — ranks by score, assigns medals, generates plain-English insights (clear leader gap, near-tie, regression-vs-classification best, undeployed notice), builds summary sentence. `_CROSS_PROJECT_PATTERNS` regex (8 NL variants) + handler + SSE emit in `chat.py`. `GET /api/projects/cross-comparison` REST endpoint registered before `/{project_id}` to prevent path capture. `CrossProjectComparisonRow` + `CrossProjectComparisonResult` TypeScript interfaces; `cross_project_comparison?` on `ChatMessage`; `attachCrossProjectComparisonToLastMessage` Zustand action; `api.projects.crossComparison()` client method; `CrossProjectComparisonCard` (indigo border, 🏆 icon): n_models badge, "without model" badge when applicable, summary paragraph, winner highlight (emerald), ranked `ProjectRow` list (rank medal/number, algorithm, target column, color-coded score bar via `ScoreBar` with role="progressbar", regression/classification `ProblemTypeBadge`, deployment status, prediction count), insights section (→ bullets), sr-only figcaption; SSE handler + render wired in workspace `page.tsx`.
      *Day 70 (20:00): 33 backend + 23 frontend = 56 new tests. Total: 4389 backend + 2477 frontend = 6866. Backend lint: clean. Frontend build: clean.*

- [x] **Auto-Insight on New Dataset** — Track E perpetual. AutoModeler automatically surfaces 1-2 specific, data-driven findings in chat on the analyst's first message after uploading a new dataset — no asking required. `compute_auto_insights(profile, col_names)` pure function in `core/analyzer.py` scans the dataset profile and returns up to 3 ranked findings: `strong_correlation` (|r|≥0.65 between two numeric columns), `date_column` (temporal patterns possible — detected via token-based column name matching, not regex `\b`), `class_imbalance` (binary categorical with ≥75% majority class), `high_missing` (column with ≥20% nulls), `high_cardinality` (likely ID column), `numeric_skew` (std > 2×mean with non-negative min). Each finding: `{insight_type, icon, finding, suggested_action, priority}` sorted ascending by priority, capped at 3. `Project.last_insight_dataset_id` field + inline SQLite migration tracks which dataset was last analyzed — handler fires exactly once per dataset. Handler block in `send_message()` guarded by `ctx["dataset"] is not None AND dataset.id != last_insight_dataset_id`; emits `{type:"auto_insight"}` SSE event; injects top findings into system_prompt for LLM narration. `AutoInsightFinding` + `AutoInsightResult` TypeScript interfaces; `auto_insight?` on `ChatMessage`; `attachAutoInsightToLastMessage` Zustand action; `AutoInsightCard` (sky border, 🔍 icon) with dataset badges, summary, finding rows (priority badge + bold-markdown finding text + action button), sr-only figcaption; SSE handler + render wired in `project/[id]/page.tsx`. Directly implements the vision's "Delightful, not just functional" and "smart colleague" principles — the system proactively says "I found something interesting in your data" without being asked.
      *Day 71 (20:00): 18 backend + 14 frontend = 32 new tests. Total: 4407 backend + 2491 frontend = 6898, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Goal Seek History via Chat** — Track B perpetual. Analysts can ask "show my goal seek history", "compare my scenarios", "list past goal seeks", "what targets have I tried?", or 7 other NL variants and receive a `GoalSeekHistoryCard` showing the last 3 goal-seek runs for the deployment, allowing comparison of alternative optimization scenarios over time. `GoalSeekRecord` SQLModel table (`id`, `deployment_id` indexed, `target_column`, `problem_type`, `algorithm_plain`, `target_value_str`, `achieved_value_str`, `achieved`, `gap_pct`, `suggestions_json` (top 5), `fixed_features_json`, `summary`, `created_at`). Every successful call to `POST /api/deploy/{id}/goal-seek` saves a record and prunes to keep only the latest `MAX_HISTORY = 3` per deployment. `GET /api/deploy/{id}/goal-seek/history` REST endpoint returns JSON with `count`, `max_history`, and `entries` list. `_GOAL_SEEK_HISTORY_PATTERNS` (9 NL variants; guarded so it does NOT also match the forward goal-seek handler) + handler in `send_message()` loads history records, injects scenario summaries into system_prompt (mentions achieved/best-effort status for each), emits `{type:"goal_seek_history"}` SSE event. `GoalSeekHistoryEntry` + `GoalSeekHistoryResult` TypeScript interfaces; `goal_seek_history?` on `ChatMessage`; `attachGoalSeekHistoryToLastMessage` Zustand action; `api.deploy.goalSeekHistory()` client method; `GoalSeekHistoryCard` (violet border, 🎯 icon): scenario-count badge, description paragraph, per-entry `EntryCard` (emerald/amber border per `achieved`, target/achieved grid, gap indicator, top-3 suggestions with ↑/↓/→ arrows and feature name + value, fixed-feature pinned list, relative timestamp), empty state with example prompts, sr-only figcaption. Closes the "I ran goal seek three times and forgot what worked" analyst gap — a smart colleague would say "last week you tried $5M and got within 3%, here's how it compared to your $6M attempt."
      *Day 72 (20:00): 12 backend + 14 frontend = 26 new tests. Total: 4465 backend + 2582 frontend = 7047, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Deployment Changelog via Chat** — Track D perpetual. Analysts can ask "what changed to my deployment?", "show my deployment changelog", "view deployment history", "when was my model last retrained?", or 6 other NL variants and receive a `DeploymentChangelogCard` showing an immutable audit trail of every lifecycle event that occurred to the deployment. `DeploymentChangelog` SQLModel table (`id`, `deployment_id` indexed, `change_type`, `description`, `created_at`). Change types: `deployed`, `redeployed`, `retrained`, `undeployed`, `api_key_added`, `api_key_removed`, plus constants for `rate_limit_set`, `quota_set`, `alert_rule_added`, `field_config_updated`, `batch_run_complete`. `_write_changelog(deployment_id, change_type, description, session)` best-effort helper (swallows all errors, never crashes callers) writes immutable entries at four key points: initial deploy (`CHANGE_DEPLOYED`), re-deploy with new model version (`CHANGE_REDEPLOYED`), undeploy (`CHANGE_UNDEPLOYED`), API key add/remove (`CHANGE_API_KEY_ADDED`/`CHANGE_API_KEY_REMOVED`). `GET /api/deploy/{id}/changelog` REST endpoint returns last `CHANGELOG_MAX_ENTRIES=50` entries newest-first, with `relative_time` ("just now"/"5m ago"/"3h ago"/"2d ago"). `_DEPLOYMENT_CHANGELOG_PATTERNS` (9 NL variants) + handler in `send_message()` loads entries from DB, injects summary of recent events into system_prompt, emits `{type:"deployment_changelog"}` SSE event. `DeploymentChangelogEntry` + `DeploymentChangelogResult` TypeScript interfaces; `deployment_changelog?` on `ChatMessage`; `attachDeploymentChangelogToLastMessage` Zustand action; `api.deploy.deploymentChangelog()` client method; `DeploymentChangelogCard` (slate border, 📋 icon): event-count badge, subtitle, timeline list with change-type-specific icon (🚀 deployed / 🔄 redeployed / 🔑 api_key_added / etc.) + color-coded badge + relative timestamp + description, `role="list"` + `aria-label="Deployment change history"` per `role="listitem"`, empty state with usage guidance, sr-only figcaption. Closes the "what happened to my deployment since I last logged in?" analyst gap — a smart colleague keeps a running log of every significant change so the analyst is never confused by unexpected model behavior after an unremembered API key rotation or retrain.
      *Day 73 (04:00): 26 backend + 16 frontend = 42 new tests. Total: 4491 backend + 2598 frontend = 7089, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Real-time SLA Latency Webhook** — Track D perpetual. When a deployment's p95 prediction latency exceeds 500ms, AutoModeler fires a registered webhook so external systems (PagerDuty, Slack, etc.) can react automatically. `EVENT_SLA_EXCEEDED = "sla_exceeded"` constant in `core/webhook.py` added to `ALL_EVENTS`. `sla_alert_last_fired_at: Optional[datetime]` field on `Deployment` model with inline SQLite migration. `_check_and_fire_sla_alert(deployment_id, threshold_ms=500.0, cooldown_hours=1.0)` in `api/deploy.py`: queries last `_SLA_WEBHOOK_N_LOGS=50` PredictionLogs, computes p95 via `_percentile()`, skips if fewer than `_SLA_WEBHOOK_MIN_SAMPLES=5` samples or p95 within threshold, checks `sla_alert_last_fired_at` cooldown gate (1h), stamps timestamp, dispatches with payload `{p95_ms, avg_ms, sample_count, threshold_ms, message}`. Wired into `make_prediction()` as a daemon background thread. `WebhookCreateBody` default `event_types` updated to include `"sla_exceeded"`. Closes the "I don't know my model is too slow until users complain" ops gap.
      *Day 73 (12:00): 20 new backend tests (`test_sla_webhook.py`). Total: 4511 backend + 2598 frontend = 7109, all passing. Backend lint: clean. Frontend build + lint: clean (backend-only change).*

- [x] **"Explain this finding" Direct-Send from AutoInsightCard** — Track E perpetual. Auto-insight action buttons previously only populated the chat input box; clicking one now directly sends the suggested action message without requiring a second click on Send. `handleSendMessage` useCallback in workspace `project/[id]/page.tsx` extended with optional `directText?: string` parameter — when provided, bypasses `chatInput` state entirely (no `setChatInput` side-effect). `AutoInsightCard` wired to invoke `onActionClick` which calls `handleSendMessage(prompt)` directly; `GoalSeekCard` receives the same `onActionClick` prop. Send button `onClick` updated to `() => handleSendMessage()` (arrow wrapper prevents React SyntheticEvent leaking into the optional param). 2 new frontend tests in `auto-insight-card.test.tsx` validating zero-delay invocation and graceful no-prop rendering.
      *Day 73 (20:00): 2 frontend tests added. Total: 4516 backend + 2613 frontend = 7129, all passing. Backend lint: clean. Frontend build + lint: clean.*
...
- [x] **Prediction Delta: "Why did my prediction change?" via Chat** — Track B perpetual. Analysts can ask "why did my prediction change after retraining?", "explain the prediction delta", "same inputs but different prediction", "why is the prediction different now?", or 5 other NL variants (8 total in `_PRED_DELTA_PATTERNS`) and receive a `PredictionDeltaCard` in chat comparing predictions between the two most recent deployment versions. `compute_prediction_delta(pipeline_path_new, model_path_new, pipeline_path_old, model_path_old, input_features)` pure function in `core/deployer.py` loads both `PredictionPipeline` + sklearn model pairs, runs both models on the same inputs, computes numeric delta/pct_change/direction (up/down/unchanged for regression; changed/unchanged for classification), uses `compute_feature_importance() × normalized deviation` attribution to compute per-feature contribution deltas, sorts by absolute contribution delta, identifies top-3 drivers, builds a plain-English summary. Returns `{old_prediction, new_prediction, delta, pct_change, direction, problem_type, target_column, provided_features, feature_delta:[{feature, old_contribution, new_contribution, contribution_delta, direction}], top_drivers, summary}`. Chat handler guarded by `ctx["deployment"]` + 2+ `DeploymentVersion` records + version_compare/cross_deploy_pred not already fired; extracts feature values from message (via `_extract_multi_feature_prediction()`) or falls back to `feature_means`; annotates result with `current_version`/`previous_version`/`inputs_from_message`; emits `{type:"prediction_delta"}` SSE event. `PredictionDeltaFeature` + `PredictionDeltaResult` TypeScript interfaces; `prediction_delta?` on `ChatMessage`; `attachPredictionDeltaToLastMessage` Zustand action; `PredictionDeltaCard` (emerald/rose/slate border per direction, 🔍 icon): version badge (v{prev}→v{curr}), direction badge (Increased/Decreased/Unchanged/Class changed), target column, old→new prediction side-by-side, delta + % change, inputs-from-message vs training-averages label, feature contribution delta horizontal bars (top 8, sorted by abs impact), top drivers amber chips, sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Closes the "same inputs but different prediction" analyst confusion after every retrain. 32 backend + 26 frontend = 58 new tests.
      *Day 75 (12:00): Implemented.*
- [x] **Predictive Cohort Monitoring via Chat** — Track B perpetual. Analysts can ask "how has my top cohort changed?", "cohort evolution", "monitor my at-risk predictions over time", or 7 other NL variants (9 total in `_COHORT_EVOLUTION_PATTERNS`) and receive a `CohortEvolutionCard` showing how the composition of the top-N prediction cohort has shifted across multiple dataset uploads. `compute_cohort_evolution(pipeline_path, model_path, dataframes, n, direction)` pure function in `core/deployer.py` loops over up to 6 most-recent `(DataFrame, dataset_id, period_label)` tuples, calls `compute_prediction_cohort()` for each, then computes period-over-period categorical shifts (only changes ≥5pp surfaced, sorted by absolute magnitude, capped at 5 per transition), builds per-transition plain-English summaries and an overall summary naming the biggest mover. Chat handler guarded by `ctx["deployment"]` + 2+ scoreable DataFrames from DB + `not cohort_event` + `not ranked_pred_event`; emits `{type:"cohort_evolution"}` SSE event. `CohortEvolutionResult`/`CohortEvolutionPeriod`/`CohortEvolutionShift`/`CohortCategoricalShift` TypeScript interfaces; `cohort_evolution?` on `ChatMessage`; `attachCohortEvolutionToLastMessage` Zustand action; `CohortEvolutionCard` (violet border, 📈 icon): periods badge, Top-N Highest/Lowest badge, target column badge, summary sentence, scrollable period timeline with `PeriodNode` (top-2 categorical profile items as mini horizontal bars) connected by `ShiftConnector` arrows (showing top shift), "Notable Composition Shifts" section with up to 6 `ShiftRow` items (emerald rising / rose falling), sr-only figcaption. Closes the "how is my cohort trending over time?" analyst gap — understanding whether the at-risk group is growing younger or shifting region is critical for proactive intervention. 47 backend + 20 frontend = 67 new tests.
      *Day 76 (04:00): Implemented.*

- [x] **Counterfactual Explanation via Chat** — Track B perpetual. Analysts can ask "what would need to change for this prediction to flip?", "counterfactual for row 5", "minimum intervention to change the prediction", "flip this prediction for row 12", "which changes would flip the prediction?", "how close is row 3 to the other class?", "what would save this customer?", "prevent churn for row 2", or 9 total NL variants (`_COUNTERFACTUAL_PATTERNS` in `chat.py`) and receive a `CounterfactualCard` showing the minimal feature changes needed to flip a classification prediction. `compute_counterfactual(pipeline_path, model_path, original_features, target_class, max_steps, step_fraction)` pure function in `core/deployer.py`: classification-only (raises `ValueError` for regression); loads PredictionPipeline + sklearn model; identifies target class (opposite of current or explicitly specified); greedy finite-difference gradient search — at each step perturbs the numeric feature whose normalized gradient toward the target probability is largest by `step_fraction × std`; stops when class flips or `max_steps` exhausted. Returns `{problem_type, target_column, original_prediction, original_class, original_confidence, counterfactual_prediction, target_class, counterfactual_confidence, flipped, n_steps, changed_features:[{name, original_value, counterfactual_value, change_pct, direction}], summary}`. Chat handler guarded by deployment + dataset + feature_set + model_runs + not local_explanation_event + not pdp_event + problem_type == "classification"; loads raw CSV row by index extracted from message or row 0 fallback, strips target column, calls `compute_counterfactual()`; injects plain-English summary into system_prompt. `CounterfactualChangedFeature` + `CounterfactualResult` TypeScript interfaces; `counterfactual?` on `ChatMessage`; `attachCounterfactualToLastMessage` Zustand action; `CounterfactualCard` (amber border when flipped / rose border when not, 🔀 icon): row index heading, target_column badge, "Flip found ✓"/"No flip found" header badge, "Classification" badge, original prediction box with confidence (`data-testid="original-prediction"`, `data-testid="original-confidence"`), counterfactual box (`data-testid="counterfactual-prediction"`, "Not reachable in N steps" when not flipped), feature changes table (Feature | Current | → | Needed | Change% with emerald/rose direction badges), empty state ("No feature changes were needed" when already at target), "firmly in class" message when not flipped with no adjustable features, `data-testid="counterfactual-summary"` paragraph, `formatValue()` helper (k/M suffix), sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Closes the "I know the prediction is bad — what specifically needs to change?" analyst gap that goal-seek doesn't answer (goal-seek optimizes a numeric target; counterfactual explains the classification boundary). 28 backend + 23 frontend = 51 new tests.
      *Day 76 (12:00): Implemented.*

- [x] **Population-Level Counterfactual via Chat** — Track B perpetual. Analysts can ask "what change would flip the most predictions?", "population counterfactual", "most impactful intervention across the cohort", "what change would help the most customers?", "single best lever for the at-risk group", "which feature flip flips the most outcomes?", "cohort intervention", or 9 total NL variants (`_POPULATION_CF_PATTERNS` in `chat.py`) and receive a `PopulationCounterfactualCard` showing which single feature intervention would flip the most predictions across a cohort. `compute_population_counterfactual(pipeline_path, model_path, input_features_list, target_class, max_rows, max_steps, step_fraction)` pure function in `core/deployer.py`: classification-only (raises `ValueError` for regression or <2 rows); loads pipeline + model once; runs greedy counterfactual for each row (up to max_rows=20); tracks primary changed feature per flip (largest absolute change); aggregates (feature, direction) → [change_pcts]; returns `{problem_type, target_column, total_rows, flipped_count, flip_rate, dominant_feature, dominant_direction, dominant_flip_count, dominant_avg_change_pct, feature_summary:[{feature, flip_count, flip_pct, avg_change_pct, direction}], summary}` sorted by flip_count desc. Chat handler guarded by deployment + dataset + feature_set + model_runs + not counterfactual_event + problem_type == "classification"; loads up to 25 CSV rows (after transformations), calls pure function, injects dominant-lever summary into system_prompt; emits `{type:"population_counterfactual"}` SSE event. `PopulationCFFeatureSummary` + `PopulationCounterfactualResult` TypeScript interfaces; `population_counterfactual?` on `ChatMessage`; `attachPopulationCounterfactualToLastMessage` Zustand action; `PopulationCounterfactualCard` (amber border when flips found / rose when none, 🎯 icon): "N of M flippable"/"No flips found" header badge, target_column + records-count badges, flip-rate region with count + progress bar (role="progressbar"), dominant intervention amber highlight ("Most Impactful Single Lever" → feature name + avg change %, dominant flip count), feature breakdown table (Feature | Rows flipped | Dir. | Avg change %, ▲/▼ arrows), no-flip empty state, summary paragraph, sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Closes the "what one change helps the most at-risk customers?" analyst gap — the cohort-level complement to per-row counterfactual, answering "if I could only change one thing for my whole at-risk group, what should it be?" 46 backend + 29 frontend = 75 new tests.
      *Day 76 (20:00): Implemented.*

- [x] **Feature Engineering Impact Analysis via Chat** — Track C perpetual. Analysts can ask "which features helped?", "did the transformations improve my model?", "feature engineering impact", "show me what the engineering added", "impact of feature engineering", "were the transformations worthwhile?", "how much did engineering help?", or 8 total NL variants (`_FE_IMPACT_PATTERNS` in `chat.py`) and receive a `FeatureEngineeringImpactCard` grouping the model's features into Original (raw columns) vs Engineered (produced by transformations) with per-group total importance bars and a plain-English verdict. `compute_feature_engineering_impact(feature_cols, importances, transformations, column_mapping)` pure function in `core/trainer.py`: builds an engineered-column set from `column_mapping` (source → new_cols from `apply_transformations` output), groups features, sorts by importance descending, computes total importance per group, derives verdict string (worthwhile/added value/minimal/none based on engineered share of total importance). Returns `{groups:[{name, total_importance, features:[{name, importance, rank, source}]}], engineered_columns, original_columns, verdict, has_engineering}`. Chat handler guarded by project + dataset + latest done model run with importances; applies transformations to get `column_mapping`, calls `compute_feature_importance()` then `compute_feature_engineering_impact()`; injects verdict into system_prompt; emits `{type:"fe_impact"}` SSE event. `GET /api/models/{run_id}/feature-engineering-impact` REST endpoint mirrors same logic. `FeatureEngineeringImpactFeature` + `FeatureEngineeringImpactGroup` + `FeatureEngineeringImpactResult` TypeScript interfaces; `fe_impact?` on `ChatMessage`; `attachFeEngineeringImpactToLastMessage` Zustand action; `FeatureEngineeringImpactCard` (violet border, ⚗️ icon): algorithm badge, engineered-feature-count badge (emerald when helpful, amber otherwise), verdict paragraph, group summary stacked bar (Original blue / Engineered violet, percentage labeled), per-group feature lists with source attribution (← source_col) and importance bars, target column footer. SSE handler + card render wired in workspace `page.tsx`. Closes the "did my feature engineering actually help?" analyst gap — analysts invest time in transformations and deserve to see whether the model actually used them.
      *Day 77 (12:00): 28 backend + 12 frontend = 40 new tests. Total: 4915 backend + 2792 frontend = 7707, all passing. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Data Quality Impact on Model Performance via Chat** — Track C perpetual. Analysts can ask "would removing outliers improve my model?", "data quality impact on model", "impact of outliers on my model", "would cleaner training data improve my accuracy?", "what if I removed the outliers from training?", "how would cleaning the data help?", or 9 total NL variants (`_DATA_QUALITY_IMPACT_PATTERNS` in `chat.py`) and receive a `DataQualityImpactCard` comparing model performance with and without anomalous training rows. `compute_data_quality_impact(X, y, algorithm, problem_type, contamination=0.05)` pure function in `core/trainer.py`: uses IsolationForest to flag the top 5% most anomalous rows, trains the model twice (full data vs. clean data, 80/20 split with same seed), computes delta in primary metric (R² for regression, accuracy for classification), derives verdict (worthwhile/marginal/no_benefit/harmful based on delta thresholds: >0.05/0.01/-0.01). Ensemble algorithms fall back to Random Forest for comparison reliability. Returns `{n_total, n_outliers, outlier_pct, baseline_score, clean_score, delta, metric_key, metric_label, verdict, improvement, recommendation, summary}`. `GET /api/models/{run_id}/data-quality-impact` REST endpoint loads the run, active feature set, and dataset; calls `prepare_features()` then `compute_data_quality_impact()`; returns 400 when <20 rows. Chat handler guarded by `ctx["model_runs"]`; selects the best/selected done run; injects verdict + delta into system_prompt; emits `{type:"data_quality_impact"}` SSE event. `DataQualityImpactResult` TypeScript interface; `data_quality_impact?` on `ChatMessage`; `attachDataQualityImpactToLastMessage` Zustand action; `DataQualityImpactCard` (emerald border=worthwhile / amber=marginal / gray=no_benefit / rose=harmful): algorithm badge, metric label badge, 3-column score grid (baseline, delta, clean) with percentage format for accuracy and decimal for R², outlier count progress bar (role="progressbar", aria-valuenow), recommendation paragraph, summary line, sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Closes the "should I clean my data before retraining?" analyst question — gives a data-backed answer rather than a generic recommendation.
      *Day 77 (20:00): 39 backend (20 pure-function + 9 pattern-match tests + 7 false-positive guards + 3 edge cases) + 22 frontend (16 card + 3 store action) = 61 new tests. Total: 4954 backend / 2814 frontend = 7768, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Overfitting/Underfitting Detection via Chat** — Track C perpetual. Analysts can ask "is my model overfitting?", "generalization gap", "is my model memorizing the training data?", "train score vs test", "model fit analysis", or 8 total NL variants (`_OVERFITTING_PATTERNS` in `chat.py`) and receive an `OverfittingAnalysisCard` comparing train score against cross-validation score with a plain-English verdict and actionable recommendations. `compute_overfitting_analysis(X, y, algorithm, problem_type, cv_folds=5)` pure function in `core/trainer.py`: fits the model on an 80/20 split to get train score, then runs `run_cross_validation()` to get cv_mean/cv_std, computes generalization gap = train_score − cv_mean. Classifies verdict: `well_fit` (|gap|<0.05), `mild_overfit` (gap>0.05), `overfit` (gap>0.15), or `underfit` (train_score below 0.30 R² / 0.60 accuracy). Returns `{train_score, cv_mean, cv_std, cv_scores, gap, gap_pct, metric_key, metric_label, verdict, verdict_label, n_rows, n_features, algorithm_plain, recommendations, summary}`. Ensemble algorithms fall back to Random Forest. `GET /api/models/{run_id}/overfitting-analysis` REST endpoint loads the run, active feature set, and dataset; calls `prepare_features()` then `compute_overfitting_analysis()`; returns 400 when <20 rows. Chat handler guarded by `ctx["model_runs"]`; selects the best/selected done run; injects verdict + gap into system_prompt; emits `{type:"overfitting_analysis"}` SSE event. `OverfittingAnalysisResult` TypeScript interface; `overfitting_analysis?` on `ChatMessage`; `attachOverfittingAnalysisToLastMessage` Zustand action; `OverfittingAnalysisCard` (emerald border=well_fit / amber=mild_overfit / rose=overfit / slate=underfit): algorithm badge, metric label badge, train vs CV 3-column grid with gap value, gap progressbar (role="progressbar"), recommendations list with → bullets, summary line, sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Closes the "is my model memorizing training data or will it generalize?" analyst question — distinct from `CvScoreDistributionCard` (which shows fold variance) and `ModelQualityScoreCard` (which scores quality) by directly answering the train/validation gap diagnostic.
      *Day 78 (04:00): 46 backend (21 pure-function + 16 regex + 3 edge cases + 6 structure) + 25 frontend (22 card + 3 store action) = 71 new tests. Total: 5000 backend / 2839 frontend = 7839, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Feature Redundancy Detection via Chat** — Track C perpetual. Analysts can ask "are any features redundant?", "multicollinearity", "which features measure the same thing?", "highly correlated features", "feature overlap", or 8 total NL variants (`_REDUNDANCY_PATTERNS` in `chat.py`) and receive a `FeatureRedundancyCard` showing which numeric feature pairs carry duplicate information and which to keep vs drop. `compute_feature_redundancy(df, feature_names, threshold=0.85)` pure function in `core/analyzer.py`: computes Pearson correlation matrix for all numeric features in `feature_names`, finds all pairs where |corr| ≥ threshold, uses union-find to cluster them into redundant groups, recommends keeping the feature with higher variance from each pair. Returns `{redundant_pairs, redundant_groups, n_redundant, n_features_checked, threshold, verdict, verdict_label, summary}`. Verdict: `none` (no pairs found), `low` (≤2 pairs), `high` (>2 pairs). `GET /api/data/{dataset_id}/feature-redundancy` REST endpoint in `api/data.py`: loads the active feature set to exclude target from feature list, returns 400 on out-of-range threshold. Chat handler guarded by `ctx["dataset"]`; loads and applies feature transformations; emits `{type:"feature_redundancy"}` SSE event with system_prompt injection naming which features to drop. `FeatureRedundancyResult` + `RedundantPair` TypeScript interfaces; `feature_redundancy?` on `ChatMessage`; `attachFeatureRedundancyToLastMessage` Zustand action; `FeatureRedundancyCard` (emerald border=none / amber=low / rose=high): verdict badge, features-checked badge, redundant-count badge, per-pair rows (feature A ↔ feature B, direction badge, correlation bar, Keep/Drop badges), drop recommendation box, summary line, sr-only figcaption. SSE handler + card render wired in workspace `page.tsx`. Distinct from `FeatureSelectionCard` (drops low-importance features based on model output) — this detects collinear features that carry *identical signal* regardless of importance.
      *Day 78 (12:00): 24 backend (18 pure-function + 2 regex + 4 endpoint) + 21 frontend (18 card component + 3 store action) = 45 new tests. Total: 5024 backend / 2860 frontend = 7884, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Target Leakage Detection via Chat** — Track C perpetual. Analysts can ask "is there target leakage?", "check for data leakage", "any leaky features?", "features that cheat the answer", "suspicious correlations with the target", or 8 total NL variants (`_TARGET_LEAKAGE_PATTERNS` in `chat.py`) and receive a `TargetLeakageCard` showing which features are suspiciously correlated with the target and may be leaking the answer into training data. `compute_target_leakage(df, target_col, feature_names, high_threshold=0.90, moderate_threshold=0.75)` pure function in `core/analyzer.py`: for numeric targets uses Pearson correlation; for categorical targets uses normalized mutual information (MI / log2(n_classes)); flags features with high-risk (|corr| ≥ 0.90) or moderate-risk (|corr| ≥ 0.75) overlap with the target. Returns `{leaky_features:[{feature, correlation, correlation_abs, risk_level, risk_label, reason}], n_checked, verdict, verdict_label, high_threshold, moderate_threshold, target_col, summary}`. Verdict: `none` (no features ≥ moderate threshold), `warning` (moderate-only), `severe` (any high-risk). `GET /api/data/{dataset_id}/target-leakage` REST endpoint in `api/data.py`: loads active FeatureSet to get target column, applies transformations, returns 400 when no target configured. Chat handler guarded by `ctx["dataset"]` + FeatureSet with target_column; applies transformations; injects leaky feature names + plain-English explanation of leakage into system_prompt; emits `{type:"target_leakage"}` SSE event. `LeakyFeature` + `TargetLeakageResult` TypeScript interfaces; `target_leakage?` on `ChatMessage`; `attachTargetLeakageToLastMessage` Zustand action; `TargetLeakageCard` (emerald border=none / amber=warning / rose=severe): verdict badge, features-checked badge, leaky-count badge, target column label, severe alert with `role="alert"`, per-feature rows (name, risk badge, correlation bar), leakage recommendation box, summary line, sr-only figcaption. SSE handler + card render wired in both EventSource branches of workspace `page.tsx`. Closes the "am I accidentally cheating with this feature?" analyst gap — a common mistake where features like `final_payment_date` or `refund_processed` encode the outcome and produce falsely great training accuracy that collapses in production.
      *Day 78 (20:00): 27 backend (15 numeric pure-function + 2 categorical + 7 regex + 3 endpoint) + 21 frontend (18 card component + 3 store action) = 48 new tests. Total: 5051 backend / 2881 frontend = 7932, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Model Confidence Distribution via Chat** — Track C perpetual. Analysts can ask "how confident is my model?", "confidence distribution", "show confidence histogram", "how certain are my predictions?", "is my model decisive or uncertain?", "distribution of prediction probabilities", "prediction confidence overview", or 8 total NL variants (`_CONFIDENCE_DIST_PATTERNS` in `chat.py`) and receive a `ConfidenceDistributionCard` showing the distribution of prediction confidence (max-class probability) across all training predictions. `compute_confidence_distribution(y_proba, y_pred, class_names, n_bins)` pure function in `core/validator.py`: buckets max-class probabilities into a histogram (5–20 bins), computes overall mean/median confidence, segments into high (≥80%), medium (50–80%), and low (<50%) tiers with counts and percentages, derives decisiveness verdict ("decisive" ≥60% high, "moderate" 30–60%, "uncertain" <30%), computes per-class mean confidence. `GET /api/models/{model_run_id}/confidence-distribution` REST endpoint in `api/validation.py` (400 for regression, 400 for non-proba models, 404 for unknown run). Chat handler guarded by classification `ctx["model_runs"]`; finds selected/best done run; loads predict_proba; calls pure function; injects decisiveness verdict + mean confidence into system_prompt; emits `{type:"confidence_distribution"}` SSE event. `ConfidenceDistributionResult` + `ConfidenceDistBin` + `ConfidenceDistClassMean` TypeScript interfaces; `confidence_distribution?` on `ChatMessage`; `attachConfidenceDistributionToLastMessage` Zustand action; `ConfidenceDistributionCard` (emerald border=decisive / amber=moderate / rose=uncertain, 🎯 icon): decisiveness badge, algorithm badge, target column, 3-column High/Medium/Low tier breakdown with counts, mean/median/N stats row, Recharts BarChart with color-coded bins (emerald=high/amber=medium/rose=low), per-class mean confidence bars with ARIA progressbars, summary paragraph, sr-only figcaption. SSE handler + card render wired in both EventSource branches of workspace `page.tsx`. Closes the "is my model typically sure about its predictions or does it often sit near 50%?" analyst gap — distinct from threshold analysis (which sweeps thresholds for precision/recall tradeoffs) and calibration check (which checks reliability of probabilities). 31 backend (16 pure-function + 11 regex + 3 endpoint integration) + 20 frontend (17 card component + 3 store action) = 51 new tests.
      *Day 79 (12:00): 31 backend + 20 frontend = 51 new tests. Total: 5113 backend / 2922 frontend = 8035, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Sample Size Adequacy Analysis via Chat** — Track C perpetual. Analysts can ask "do I have enough training data?", "is my dataset big enough?", "how many more rows do I need?", "is my sample size sufficient?", "sample size check", "is my data too small?", "do I have enough data to train?", or "minimum required samples", 8 total NL variants (`_SAMPLE_SIZE_PATTERNS` in `chat.py`) and receive a `SampleSizeAdequacyCard` in chat with a verdict on whether training data volume meets the 10× rule of thumb for the model's complexity. `compute_sample_size_adequacy(n_rows, n_features, problem_type, n_classes, cv_std)` pure function in `core/analyzer.py`: regression recommendation = max(50, 10×n_features); classification = max(50×n_classes, 10×n_features×n_classes). Verdicts: `adequate` (n_rows ≥ recommended), `borderline` (60–99%), `insufficient` (<60%). Computes feature/row ratio, coverage_pct, shortfall, and optionally validates against cv_std (cv_stable = True if cv_std ≤ 0.10). `GET /api/models/{run_id}/sample-size-adequacy` REST endpoint in `api/models.py`: loads run, feature set, and dataset; calls `prepare_features()` to get n_rows/n_features; reads cv_std from stored metrics. Chat handler guarded by `ctx["model_runs"]`; selects best/selected done run; injects verdict + shortfall into system_prompt; emits `{type:"sample_size_adequacy"}` SSE event. `SampleSizeAdequacyResult` TypeScript interface; `sample_size_adequacy?` on `ChatMessage`; `attachSampleSizeAdequacyToLastMessage` Zustand action; `SampleSizeAdequacyCard` (emerald border=adequate / amber=borderline / rose=insufficient): verdict badge, algorithm badge, problem-type badge, 3-column grid (Current Rows / Recommended / Shortfall), coverage progress bar (role="progressbar"), feature/row ratio badge (amber when ratio > 0.1), optional CV stability badge, recommendations list, summary paragraph, sr-only figcaption. SSE handler + card render wired in both EventSource branches of workspace `page.tsx`. Closes the "do I have enough data to trust this model?" analyst gap — a common concern for analysts who upload 100-row spreadsheets and wonder whether the results are reliable. 35 backend (21 pure-function + 11 regex + 3 endpoint integration) + 21 frontend (18 card component + 3 store action) = 56 new tests.
      *Day 79 (20:00): 35 backend + 21 frontend = 56 new tests. Total: 5148 backend / 2943 frontend = 8091, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Class-Conditional Feature Importance via Chat** — Track C perpetual. Analysts can ask "what features drive churn predictions?", "feature importance per class", "class-conditional feature importance", "class-specific feature breakdown", "what makes the model predict X vs Y?", "feature breakdown by class", "why does the model predict different classes?", or "what features are different for each class?", 8 total NL variants (`_CLASS_FEAT_IMP_PATTERNS` in `chat.py`) and receive a `ClassFeatureImportanceCard` showing which features are most systematically different for each predicted class — answering "what makes the model say 'churn' vs 'not churn'?" `compute_class_conditional_importance(model, X, y_pred, feature_names, class_names)` pure function in `core/explainer.py`: for each predicted class, filters training rows to that cohort, computes per-feature deviations from the global mean (weighted by feature importance), derives direction labels (above_average/below_average/similar based on 5% threshold), sorts by weighted_deviation descending (top 8 shown), builds per-class summary sentence naming the top distinguishing feature and its deviation. Classification only; raises ValueError for fewer than 10 rows or a single predicted class. `GET /api/models/{run_id}/class-feature-importance` REST endpoint in `api/validation.py` (400 for regression, 404 for unknown run). Chat handler guarded by classification `ctx["model_runs"]`; selects best/selected done run; loads CSV, transforms, calls `compute_class_conditional_importance()`; injects per-class highlights into system_prompt; emits `{type:"class_feature_importance"}` SSE event. `ClassFeatureEntry` + `ClassImportanceGroup` + `ClassFeatureImportanceResult` TypeScript interfaces; `class_feature_importance?` on `ChatMessage`; `attachClassFeatureImportanceToLastMessage` Zustand action; `api.models.classFeatureImportance()` client method; `ClassFeatureImportanceCard` (indigo border, 🎯 icon): n-classes badge, algorithm badge, target column badge, n-samples badge, explainer note, per-class collapsible `ClassPanel` (expand/collapse toggle with `aria-expanded`, class label badge, sample count + percentage, feature rows with proportional bars and direction labels ↑/↓/≈), overall summary paragraph, sr-only figcaption. First class expands by default; clicking opens/closes. Closes the "what specifically makes the model predict each outcome?" analyst gap — distinct from global feature importance (averages all predictions), PDP (population-level marginal effect), and local explanation (single row). A business analyst would say: "I can see my model says 'churn' because those customers have higher monthly charges and shorter tenure — which makes business sense." 32 backend (17 pure-function + 4 error/edge cases + 4 regex + 3 endpoint integration) + 20 frontend (17 card component + 3 store action) = 52 new tests.
      *Day 80 (04:00): 32 backend + 20 frontend = 52 new tests. Total: 5180 backend / 2963 frontend = 8143, all passing. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Calibration Check via Chat** — Track C perpetual. Analysts can ask "is my model calibrated?", "calibration check", "are my probabilities reliable?", "can I trust my probability scores?", "what's the brier score?", "reliability diagram", "probability calibration", or "are my confidence scores accurate?", 10 total NL variants (`_CALIBRATION_CHECK_PATTERNS` in `chat.py`) and receive a `CalibrationCheckCard` directly answering "when my model says 70% probability, is it actually correct ~70% of the time?" `compute_calibration_check(y_true, y_proba_matrix, class_names, n_bins)` pure function in `core/validator.py`: uses sklearn's `calibration_curve` to build a reliability diagram (actual fraction of positives vs mean predicted probability per bin). Binary: uses positive-class probability column; multiclass: uses max-class confidence (accuracy-calibration) plus one-vs-rest per-class Brier scores. Computes ECE (Expected Calibration Error = weighted average of |predicted − actual fraction| per bin); overall Brier score via `brier_score_loss`; verdicts: `well_calibrated` (ECE < 0.05), `moderate` (ECE 0.05–0.15), `poorly_calibrated` (ECE ≥ 0.15). Returns `{curve_points:[{mean_prob, frac_positive, count, bin_label}], ece, brier_score, verdict, verdict_label, per_class:[{class_name, brier_score, ece, n_positive}], n_classes, n_total, summary}`. `GET /api/models/{model_run_id}/calibration-check?n_bins=10` REST endpoint in `api/validation.py` (400 for regression or non-proba models, 404 for unknown run). Chat handler guarded by classification `ctx["model_runs"]`; finds selected/best done run; loads predict_proba; calls pure function; injects ECE + verdict into system_prompt with guidance that "a well-calibrated model's probability scores reliably match real-world likelihoods — useful when business decisions depend on the probability value itself, not just the predicted class"; emits `{type:"calibration_check"}` SSE event. `CalibrationCurvePoint` + `CalibrationPerClassEntry` + `CalibrationCheckResult` TypeScript interfaces; `calibration_check?` on `ChatMessage`; `attachCalibrationCheckToLastMessage` Zustand action; `api.models.calibrationCheck()` client method; `CalibrationCheckCard` (emerald border=well_calibrated / amber=moderate / rose=poorly_calibrated, 📐 icon): verdict badge, algorithm badge, target column badge, n_classes badge, n_total badge, ECE/Brier score 2-column grid, Recharts LineChart reliability diagram with actual calibration line + dashed perfect-calibration reference, per-class table (multiclass only), plain-English summary paragraph, sr-only figcaption. Closes the "can I trust my model's probability output for business decisions?" analyst gap — distinct from `ConfidenceDistributionCard` (distribution of confidence levels) and `ThresholdAnalysisCard` (precision/recall tradeoffs at different thresholds). 20 backend pure-function tests + 10 regex tests + 3 REST endpoint integration tests = 33 backend; 15 card component + 3 store action = 18 frontend = **51 new tests**. Backend lint: clean. Frontend build + lint: clean.
      *Day 80 (12:00): 33 backend + 18 frontend = 51 new tests. Total: 5200 backend / 2966 frontend, all passing. Replaced stale test_calibration_check.py that tested a metrics-injection approach with pure-function + REST + regex tests matching the live-compute implementation.*

- [x] **Prediction Error Correlation via Chat** — Track C perpetual. Analysts can ask "which features correlate with my model's errors?", "what causes my model's prediction errors?", "error correlation", "where does my model systematically struggle?", "feature correlation with errors", "what drives my model's errors?", "which input features cause more errors?", or "drivers of my prediction errors", 8 total NL variants (`_ERROR_CORRELATION_PATTERNS` in `chat.py`) and receive an `ErrorCorrelationCard` showing which input features are most correlated with the magnitude of prediction errors — answering "where does my model consistently go wrong and what input signal predicts that?" `compute_prediction_error_correlation(X, y_true, y_pred, feature_names, problem_type)` pure function in `core/validator.py`: for regression computes error vector = |y_pred − y_true| (absolute residuals); for classification computes error vector = (y_pred ≠ y_true).astype(int) (misclassification indicator). Computes Pearson correlation of each feature column with the error vector, sorts by |correlation| descending, caps at top 10 features, derives direction labels (positive = higher feature value → more errors; negative = lower → more errors; neutral = |r| ≤ 0.05). Verdicts: `clear_drivers` (max |r| ≥ 0.30), `weak_drivers` (max |r| ≥ 0.10), `none` (all |r| < 0.10). Returns `{features:[{feature, correlation, correlation_abs, direction, rank}], error_type, n_total, n_errors, error_rate, verdict, verdict_label, top_driver, summary}`. `GET /api/models/{model_run_id}/error-correlation` REST endpoint in `api/validation.py` (404 for unknown run). Chat handler guarded by `ctx["model_runs"]`; selects best/selected done run; applies feature transformations; loads joblib model; calls `compute_prediction_error_correlation()`; injects top_driver + verdict into system_prompt with narration guidance; emits `{type:"error_correlation"}` SSE event. `ErrorCorrelationFeature` + `ErrorCorrelationResult` TypeScript interfaces; `error_correlation?` on `ChatMessage`; `attachErrorCorrelationToLastMessage` Zustand action; `ErrorCorrelationCard` (rose border=clear_drivers / amber=weak_drivers / emerald=none, 🔎 icon): verdict badge, algorithm badge, target column badge, stats row (n_total / n_errors / error_rate for classification), feature list with rank, correlation bar (rose for positive direction, sky for negative, muted for neutral), r-value monospace, direction arrow (↑/↓/≈), `top-driver-callout` div (rose background, visible only for `clear_drivers`), summary paragraph, sr-only figcaption. Closes the "where does my model systematically fail and what input explains it?" analyst gap — distinct from `ErrorDistributionCard` (histogram of error magnitudes), `PredictionErrorCard` (top-N worst rows), and `SegmentPerformanceCard` (accuracy within one specific feature's segments) by directly answering the causal question of which feature values the model struggles with across its entire training set. 33 backend (19 pure-function + 8 regex + 4 false-positive guards + 2 endpoint integration) + 20 frontend (18 card component + 2 store action) = **53 new tests**. Backend lint: clean. Frontend build + lint: clean.
      *Day 80 (20:00): Implemented.*

- [x] **Prediction Output Anomaly Detection via Chat** — Track D perpetual. Analysts can ask "any unusual predictions?", "prediction outliers", "anomalous outputs", "weird model outputs", "predictions that are unusually high", or "suspicious model outputs" (8 total NL variants in `_OUTPUT_ANOMALY_PATTERNS`) and receive a `PredictionOutputAnomalyCard` showing which recent production predictions are statistically extreme (regression) or unusually low-confidence (classification). `compute_prediction_output_anomalies(logs, problem_type, z_score_threshold=2.5, confidence_threshold=0.55, max_anomalies=10)` pure function in `core/analyzer.py`: regression path extracts `prediction_numeric` values, computes population mean/std, flags rows with |z-score| > threshold; classification path extracts `confidence` values, flags rows below confidence_threshold. Both paths sort anomalies (z-score desc / confidence asc), cap at max_anomalies, and compute anomaly_rate. Verdicts: `no_anomalies` (rate=0), `few_anomalies` (<10%), `many_anomalies` (≥10%). Each anomaly entry: id (8-char), prediction_value, deviation (e.g. "2.9σ above mean"), reason (plain English), input_summary (first 3 features from input_features JSON). Handles all-identical regression predictions (std=0) → no_anomalies. Raises ValueError for <5 logs. `GET /api/deploy/{id}/output-anomalies?n=50` REST endpoint in `api/deploy.py`: loads last N PredictionLogs, returns enriched result with deployment_id; returns `no_data` verdict when <5 logs. Chat handler guarded by `ctx["deployment"]`; loads 50 PredictionLogs; emits `{type:"output_anomalies"}` SSE event. `PredictionOutputAnomalyCard` (rose=many_anomalies / amber=few_anomalies / emerald=no_anomalies / gray=no_data, 🔍 icon): verdict badge, problem-type badge, n-total badge, stats grid (regression: mean/std/anomaly-count; classification: mean/min/anomaly-count), per-anomaly `AnomalyRow` (prediction value, deviation badge, z-score/confidence, reason, relative time, input feature chips), "All within range" success state when no anomalies, summary paragraph, sr-only figcaption. Distinct from: `AnomalyCard` (input anomalies via IsolationForest), `CovariateDriftAlertCard` (input distribution shifts), `PredictionErrorCard` (training-time wrong predictions), `ConfidenceDistributionCard` (aggregate spread). Closes the "are my production predictions making sense?" gap — the first feature to inspect the *output values* of live predictions for suspicious patterns that the analyst can investigate.
      *Day 81 (04:00): 34 backend + 22 frontend = 56 new tests. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Prediction Output Distribution Shift via Chat** — Track D perpetual. Analysts can ask "has the distribution of my predictions shifted?", "output distribution shift", "are my predictions shifting over time?", "how has my model's output changed?", "production prediction distribution", "prediction value drift", "compare training vs production predictions distributions", or "are my model outputs behaving differently in production?" (8 total NL variants in `_OUTPUT_DIST_SHIFT_PATTERNS`) and receive a `PredictionOutputDistributionCard` comparing the *statistical distribution* of production predictions vs training-time predictions using a Kolmogorov-Smirnov test. `compute_prediction_output_distribution_shift(training_preds, production_preds, n_bins=10)` pure function in `core/analyzer.py`: runs `scipy.stats.ks_2samp()` on both arrays; computes per-distribution stats (mean/std/min/p25/median/p75/p95/max); derives `mean_shift` (production_mean − training_mean) and `mean_shift_pct`; builds aligned histograms with shared bin edges for visual overlay; computes `std_ratio`. Verdicts: `significant_shift` (KS p-value < 0.01 or |mean_shift_pct| > 30%), `moderate_shift` (p < 0.05 or |shift| > 10%), `stable` (otherwise). Raises ValueError for <10 samples in either list. `GET /api/deploy/{id}/output-distribution-shift?n=100` REST endpoint in `api/deploy.py`: loads last N PredictionLogs for production_preds; finds most-recent Dataset via project_id, then FeatureSet, loads joblib pipeline + model to generate training_preds by re-predicting the training CSV; returns `no_data` when <10 production predictions available. Chat handler guarded by `ctx["deployment"]`; inline pipeline load + predict; injects KS statistic + mean_shift_pct + verdict into system_prompt; emits `{type:"output_distribution_shift"}` SSE event. `PredictionOutputDistributionCard` (emerald=stable / amber=moderate_shift / rose=significant_shift / gray=no_data, 📊 icon): verdict badge, problem-type badge, n-counts badge, KS statistic + p-value + mean shift stats row, Distribution Comparison table (training vs production stats side-by-side), Recharts BarChart histogram overlay (gray=training, indigo=production), plain-English summary, sr-only figcaption. `PredictionDistributionStats` + `PredictionDistHistogramBin` + `PredictionOutputDistributionShiftResult` TypeScript interfaces; `output_distribution_shift?` on `ChatMessage`; `attachOutputDistributionShiftToLastMessage` Zustand action; SSE handler + card render in both EventSource branches of `project/[id]/page.tsx`. Distinct from: `DriftCard` (compares *input* feature distributions with z-score/TVD), `PredictionOutputAnomalyCard` (individual outlier predictions), `compute_training_vs_production` (requires labeled feedback to compare *accuracy*). Closes the "is my model producing systematically different output values without me knowing?" gap — works without labeled ground truth, purely from prediction values.
      *Day 81 (12:00): 37 backend + 24 frontend = 61 new tests. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Feature PSI Ranking via Chat** — Track D perpetual. Analysts can ask "which features have drifted the most?", "feature psi analysis", "psi analysis for my features", "rank my features by drift", "most drifted input features", "feature drift ranking", "how much have my input features changed?", or "feature stability analysis" (8 NL variants in `_FEATURE_PSI_PATTERNS`) and receive a `FeaturePsiCard` ranking ALL input features by their Population Stability Index (PSI) between training and production. `compute_feature_psi_ranking(training_df, production_inputs, feature_names, n_bins, max_features)` pure function in `core/analyzer.py`: detects feature type (numeric/categorical); for numeric: builds equal-frequency bins from training percentiles (capped to ensure ≥10 production samples per bin for stability), computes PSI = Σ(prod_pct - train_pct) × ln(prod_pct / train_pct) with ε=1e-4 zero-guard; for categorical: computes per-category PSI across the union of training + production categories, returns n_new_categories and n_dropped_categories. Features sorted by PSI descending. Verdicts per feature: `stable` (PSI < 0.10), `watch` (0.10–0.20), `critical` (≥ 0.20). Overall verdict = worst feature. Returns features list, features_analyzed, stable/watch/critical counts, overall_psi (mean), sample_count, training_count, summary. Raises ValueError for <10 training rows or empty production_inputs. `GET /api/deploy/{id}/feature-psi?n=200` REST endpoint in `api/deploy.py`: loads last N PredictionLog input_features (JSON-parsed), fetches training Dataset + FeatureSet, reads CSV, drops target column, aligns feature names to production keys, calls pure function, returns result with deployment_id/algorithm/target_col. Returns no_data verdict when <10 production inputs available. Chat handler guarded by `ctx["deployment"]`; loads 200 PredictionLogs; aligns feature names; injects verdict + critical/watch/stable counts + top drifted feature into system_prompt; emits `{type:"feature_psi"}` SSE event. `FeaturePsiCard` (adaptive border by verdict, 📊 icon): verdict badge, features_analyzed badge, n=sample_count badge; 3-stat summary grid (critical/watch/stable counts); ranked feature list with horizontal PSI bar (rose/amber/emerald by severity), PSI value, severity badge, categorical new-categories annotation; overall mean PSI row; PSI legend (Stable <0.10 / Watch 0.10–0.20 / Critical ≥0.20); plain-English summary paragraph; sr-only figcaption. `FeaturePsiEntry` + `FeaturePsiResult` TypeScript interfaces; `feature_psi?` on `ChatMessage`; `attachFeaturePsiToLastMessage` Zustand action; SSE handler + card render wired in both EventSource branches of `project/[id]/page.tsx`. Distinct from: `CovariateAlertCard` (OOR-based binary alert per feature using training min/max), `DriftCard` (aggregate dataset comparison), `OutputDistributionShiftCard` (output value distribution shift). PSI is the industry-standard metric for measuring how much the statistical distribution of a feature has shifted — not just whether values are out of range, but whether the entire shape of the distribution has changed.
      *Day 82 (04:00): 35 backend (22 pure-function + 8 regex + 3 REST) + 19 frontend (16 card + 3 store) = 54 new tests. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Minimum Viable Feature Set via Chat** — Track C perpetual. Analysts can ask "what's the minimum features needed?", "which features can I drop?", "simplify my model", "reduce my feature set", "feature pruning", "fewest features for good predictions", "which features are redundant?", or "can I drop any features?" (8 NL variants in `_MIN_FEATURE_SET_PATTERNS`) and receive a `MinFeatureSetCard` showing the result of greedy backward elimination — the smallest feature subset that preserves model accuracy within a 2% tolerance. `compute_min_viable_feature_set(X, y, feature_names, model_class, model_params, problem_type, tolerance=0.02, max_rows=2000, cv=3)` pure function in `core/validator.py`: sub-samples to max_rows for speed; computes baseline CV score (R² for regression, accuracy for classification) with all features; fits the model once to extract feature importances (feature_importances_ for tree models, coef_ for linear models); sorts features by importance ascending (least important first); greedily removes the least-important remaining feature and re-evaluates CV score — drops permanently if score loss ≤ tolerance, stops as soon as a removal would exceed tolerance. Returns: `features` list (each with feature/importance/kept/rank), `n_original`, `n_minimal`, `features_dropped`, `features_retained` list, `features_dropped_list`, `baseline_score`, `minimal_score`, `score_loss`, `can_simplify` (True when ≥1 feature dropped), `reduction_pct`, `metric`, `summary`. Raises ValueError for <10 rows or <2 features. `GET /api/models/{model_run_id}/min-feature-set?tolerance=0.02` REST endpoint in `api/validation.py`: loads run context via `_load_run_context`, calls `_build_Xy`, looks up model_class/params from algorithm registry, calls pure function, returns result with model_run_id/algorithm/target_col. Returns 404 for unknown run, 400 for invalid tolerance. Chat handler guarded by `ctx["model_runs"]`; selects the selected or first completed run; loads CSV, applies transforms, calls pure function; injects n_original, n_minimal, features_dropped, score_loss, can_simplify, and summary into system_prompt; emits `{type:"min_feature_set"}` SSE event. `MinFeatureSetCard` (sky border when can_simplify, slate otherwise, ✂️ icon): algorithm + target badges, droppable/all-needed badge; score comparison grid (baseline score, minimal score, score loss colored emerald/amber/rose); ranked feature list with ✓/✗ indicator, optional strikethrough for dropped features, horizontal importance bar (sky=kept, slate=dropped); retained/dropped two-column grid (sky box for retained, slate box for dropped); reduction pct stat row; summary paragraph; sr-only figcaption. Formats scores as R² decimal or accuracy percentage depending on metric. `MinFeatureEntry` + `MinFeatureSetResult` TypeScript interfaces; `min_feature_set?` on `ChatMessage`; `attachMinFeatureSetToLastMessage` Zustand action; SSE handler + card render wired in both EventSource branches of `project/[id]/page.tsx`. Directly closes the "my prediction form has 20 fields — which ones actually matter?" analyst gap, enabling a simpler, more robust deployed model.
      *Day 82 (12:00): 35 backend (23 pure-function + 10 regex + 2 REST) + 20 frontend (18 card + 2 store) = 55 new tests. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Retraining Readiness Assessment via Chat** — Track D perpetual. Analysts can ask "should I retrain my model?", "retrain recommendation", "retraining readiness", "is my model degrading?", "do I need to retrain?", "comprehensive model health check", or "model degradation score" (9 NL variants in `_RETRAIN_READINESS_PATTERNS`) and receive a `RetrainingReadinessCard` showing a composite 0-100 urgency score that aggregates all available monitoring signals into a single retraining recommendation. `compute_retraining_readiness(age_days, anomaly_rate, confidence_trend, feedback_verdict, psi_critical_count, output_shift_verdict)` pure function in `core/analyzer.py`: each signal contributes a weighted sub-score normalized to 0-100 — model age (max 40 pts: 0 if <30 days, 40 if >180 days), prediction anomaly rate (max 30 pts: 30 if ≥15%, 15 if ≥5%), confidence trend (max 20 pts: 20 if declining), feedback accuracy verdict (max 30 pts: 30 if poor, 15 if moderate), PSI critical features (max 25 pts: 25 if ≥2 critical, 15 if 1), output distribution shift (max 25 pts: 25 if significant, 10 if moderate). Raw score normalized by total possible from available signals. Verdicts: `stable` (<30), `monitor` (30-59), `retrain_soon` (60-79), `retrain_now` (≥80). Returns score, verdict, verdict_label, signals list (each with name/label/value_str/score_contribution/is_firing/detail), signals_available, signals_firing, top_reason, recommendations, summary. When no signals available, returns score=0 + guidance to collect more data. `GET /api/deploy/{id}/retraining-readiness?n=100` REST endpoint in `api/deploy.py`: computes age_days from ModelRun.created_at, anomaly_rate from z-scores in recent PredictionLog.prediction_numeric (regression) or confidence trend from PredictionLog.confidence (classification), feedback_verdict from FeedbackRecord via `_compute_feedback_accuracy_simple()`, omits PSI and output shift signals at REST level (too expensive to compute inline). Chat handler guarded by `ctx["deployment"]`; computes the same signals directly from session queries; emits `{type:"retraining_readiness"}` SSE event; injects score+verdict+top_reason into system_prompt. `RetrainingReadinessCard` (emerald border=stable / amber=monitor / orange=retrain_soon / rose=retrain_now): verdict badge, score/100 badge, signals-firing badge; score progress bar (role="progressbar"); per-signal rows with ✓/! indicator, label, value string, detail text, contribution bar for firing signals; recommendations list with → bullets; "Retrain Model Now" CTA button (fires `onActionClick("retrain my model")`) shown only for retrain_soon/retrain_now when handler provided; summary paragraph; sr-only figcaption. Distinct from: `ProjectHealthCard` (age+usage health score only, not quality signals), `PredictionAuditCard` (raw stats card), individual drift/anomaly/PSI cards (single-signal views). Closes the "I have all these monitoring cards but which one actually tells me when to retrain?" analyst gap — a smart colleague who sees declining confidence + high anomaly rate + aging model would say "it's time to retrain" without making the analyst synthesize six separate cards.
      *Day 82 (20:00): 48 backend (28 pure-function + 13 regex + 3 REST + 4 endpoint integration) + 18 frontend (16 card + 2 store) = 66 new tests. Backend lint: clean. Frontend build + lint: clean.*

- [x] **Production Prediction Value Trend via Chat** — Track D perpetual. Analysts can ask "are my predictions trending up?", "prediction value trend", "show me how my predictions have changed over time", "are my model outputs getting higher over time?", "regression output trend", "prediction value history", or "show prediction time series" (8 NL variants in `_PRED_VALUE_TREND_PATTERNS`) and receive a `PredictionValueTrendCard` in chat showing a time-series chart of mean regression prediction values grouped by day. `compute_prediction_value_trend(logs_data, period, n_periods)` pure function in `core/analyzer.py`: filters to numeric regression predictions, floors timestamps to the requested period (day or week), groups values into buckets by period, computes per-period mean/count/min/max, uses numpy polyfit (degree 1) to derive slope, derives `overall_change_pct = (last_mean − first_mean) / |first_mean| × 100`, assigns direction verdicts — `trending_up` (>+5%), `trending_down` (<-5%), `stable` (otherwise). Handles ISO-format string timestamps; skips None prediction_numeric; caps to n_periods most-recent periods. Raises ValueError for <2 periods with data or no numeric predictions. Returns periods list (each with period_label/period_start/mean/count/min/max), n_total, n_periods_with_data, direction, direction_label, slope, slope_pct_per_period, first_period_mean, last_period_mean, overall_change_pct, summary. `GET /api/deploy/{id}/prediction-value-trend?period=day&n=200` REST endpoint in `api/deploy.py`: returns not_applicable for classification deployments; returns no_data verdict when < 2 periods available. Chat handler guarded by `ctx["deployment"]` and `problem_type == "regression"`; loads 200 PredictionLogs in ascending order; injects direction + overall_change_pct + summary into system_prompt; emits `{type:"prediction_value_trend"}` SSE event. `PredictionValueTrendCard` (emerald=trending_up / rose=trending_down / amber=stable / gray=no_data, 📈 icon): direction badge with directional arrow (↗/↘/→), first/last period average stats, net-change stat (colored by direction), Recharts LineChart with period means (colored line: emerald/rose/amber), period count + n_total predictions footer, algorithm name, summary paragraph, sr-only figcaption. Distinct from: `PredictionOutputDistributionCard` (overall statistical distribution shape change using KS test), `PredictionOutputAnomalyCard` (individual outlier predictions). Answers the "are my model's outputs systematically drifting upward or downward over time?" question — the simplest directional signal an analyst can share with a VP.
      *Day 83 (04:00): 38 backend (21 pure-function + 10 regex + 2 REST endpoint + 4 false-positive guards + 1 edge case) + 18 frontend (16 card component + 2 store action) = 56 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Deployment Monitoring Signal Digest via Chat** — Track B perpetual. Analysts can ask "show all my monitoring signals", "monitoring signal digest", "deployment diagnostics", "give me a full monitoring overview", "all my monitoring signals", "comprehensive monitoring check", "what are all my monitoring signals", or "monitoring signals at a glance" (8 NL variants in `_MONITORING_DIGEST_PATTERNS`) and receive a `MonitoringDigestCard` in chat — a "mission control" compact overview of ALL active monitoring signal verdicts in a single card, without having to ask 5 separate questions. `compute_deployment_monitoring_digest(anomaly_result, value_trend_result, dist_shift_result, readiness_result, usage_7d, usage_prev_7d, problem_type)` pure function in `core/analyzer.py`: aggregates up to 5 pre-computed signal results — output anomalies (no_anomalies/few/many → green/amber/red), prediction value trend (stable=green / trending=amber; regression only), output distribution shift (stable/moderate/significant → green/amber/red), retraining readiness (stable/monitor=green|amber / retrain_soon=amber / retrain_now=red), and usage activity (active/low/none → green/amber/amber). Each signal produces a `{signal_key, signal_label, verdict, verdict_label, severity, finding, icon, is_available}` entry. Overall health = critical (≥2 red) / warning (1 red) / watching (≥1 amber, 0 red) / healthy (all green). Overall score = 100 − red×25 − amber×10. Priority actions list (up to 3): drawn from retraining recommendations, dist-shift callouts, anomaly investigation notes, usage alerts. `GET /api/deploy/{id}/monitoring-digest?n=100` REST endpoint in `api/deploy.py`: computes all signals inline (anomalies, value trend, dist shift via pipeline reload, retraining readiness) and returns digest with deployment_id/algorithm/target_col. Chat handler guarded by `ctx["deployment"]`; computes all signals inline; injects overall_health + score + firing count + summary into system_prompt; emits `{type:"monitoring_digest"}` SSE event. `MonitoringDigestCard` (blue=healthy / amber=watching / orange=warning / rose=critical): overall health badge, score/100 badge, signals-need-attention badge, target column badge; signal list with role="list" (each row: icon + label + colored status dot + verdict_label + finding text, background-tinted by severity); priority actions amber box (→ bullets); summary paragraph; sr-only figcaption. Distinct from: `ModelStatusReportCard` (download-only HTML report for stakeholders, covers basic health+volume+SLA), `RetrainingReadinessCard` (single retrain decision score), `ProjectHealthCard` (age+usage health only). Closes the "I have 6 monitoring cards but need to ask 6 questions to get a complete picture" analyst gap — the single-pane-of-glass monitoring view. `MonitoringDigestSignal` + `MonitoringDigestResult` TypeScript interfaces; `monitoring_digest?` on `ChatMessage`; `attachMonitoringDigestToLastMessage` Zustand action; SSE handler + card render wired in both EventSource branches of `project/[id]/page.tsx`.
      *Day 83 (12:00): 34 backend (24 pure-function + 10 regex) + 17 frontend (15 card component + 2 store action) = 51 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Production Feedback Threshold Optimizer via Chat** — Track C perpetual. Analysts can ask "what confidence threshold maximizes my production F1?", "optimize my classification threshold from feedback", "best threshold based on actual outcomes", "threshold optimizer from production", "production feedback threshold optimization", "real-world threshold analysis", "what threshold should I use based on actual outcomes", or "calibrate my decision threshold using feedback" (8 NL variants in `_PROD_THRESHOLD_OPT_PATTERNS`) and receive a `ProductionThresholdOptimizerCard` showing the optimal confidence threshold derived from real `FeedbackRecord` outcomes. Distinct from `ThresholdAnalysisCard` (training-time data only) — this uses actual production labels recorded by analysts after predictions. `compute_production_threshold_optimizer(feedback_pairs)` pure function in `core/validator.py`: sweeps 19 confidence thresholds (0.05–0.95), computing precision (accuracy of served predictions), recall (fraction of correct predictions served), F1, and coverage at each threshold; finds the threshold that maximizes production F1. `GET /api/deploy/{id}/production-threshold-optimizer` REST endpoint in `api/deploy.py`: loads `FeedbackRecord` entries with `actual_label` + `prediction_log_id`, joins to `PredictionLog` for confidence scores, calls pure function. Returns `no_data` when < 5 usable pairs; `not_applicable` for regression. Chat handler guarded by `ctx["deployment"]` + classification `problem_type`; builds feedback_pairs inline; injects `optimal_threshold + optimal_f1 + n_feedback` into system_prompt; emits `{type:"production_threshold_optimizer"}` SSE event. `ProductionThresholdOptimizerCard` (amber=improved / emerald=same / gray=no_data): verdict badge, n_feedback outcomes badge, algorithm badge; optimal-threshold stat grid (F1/precision/recall/coverage); comparison row (overall accuracy / current F1 @ 50% / F1 gain); Recharts LineChart with F1/precision/recall sweep + dashed optimal reference line; summary paragraph; sr-only figcaption. `ProductionThresholdSweepPoint` + `ProductionThresholdOptimizerResult` TypeScript interfaces; `production_threshold_optimizer?` on `ChatMessage`; `attachProductionThresholdOptimizerToLastMessage` Zustand action; SSE handler + card render in `project/[id]/page.tsx`.
      *Day 83 (20:00): 36 backend (25 pure-function + 10 regex + 1 REST 404 + 1 REST no_data) + 17 frontend (15 card + 2 store action) = 53 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Automated Weekly Monitoring Digest Webhook** — Track D perpetual. Analysts can say "enable weekly monitoring digest", "schedule a weekly monitoring report", "send me a weekly digest", "disable the weekly digest", or "show my weekly digest status" (8 NL variants in `_WEEKLY_DIGEST_PATTERNS`) and configure AutoModeler to automatically compute the full monitoring signal digest every week and dispatch it to registered webhooks — no manual asking required. `WeeklyDigestConfig` SQLModel table (`id`, `deployment_id` indexed, `enabled`, `day_of_week` (0=Monday), `send_hour` (UTC), `last_sent_at`). `should_send_weekly_digest(day_of_week, send_hour, last_sent_at, now)` pure function: fires when today matches the configured day, current hour ≥ send_hour, and not already sent today. `_run_weekly_digest(config_id)` in `core/scheduler.py`: loads deployment + recent PredictionLogs, calls `compute_deployment_monitoring_digest()` inline, stamps `last_sent_at`, dispatches via `EVENT_WEEKLY_DIGEST` webhook (registered in `ALL_EVENTS`). `_scheduler_loop()` extended to query enabled digest configs and fire them in daemon threads alongside batch jobs. REST: `GET/PUT /api/deploy/{id}/weekly-digest-config` (day + time + enabled flag) + `DELETE` to remove. Chat handler: `_WEEKLY_DIGEST_ENABLE_RE` / `_WEEKLY_DIGEST_DISABLE_RE` / status intent; parses day-of-week name + hour from message; emits `{type:"weekly_digest_config"}` SSE event. `WeeklyDigestConfigCard` (teal border, 📅 icon): Enabled/Disabled badge, day/time grid, last-sent date, webhook explanation note. `WeeklyDigestConfigResult` TypeScript type; `attachWeeklyDigestConfigToLastMessage` Zustand action; SSE handler + render wired in workspace `page.tsx`. Closes the "passive monitoring" gap — analysts who configured monitoring signals no longer need to remember to check them weekly; the system proactively dispatches a complete health report to Slack/Teams/PagerDuty/Zapier every Monday morning.
      *Day 84 (12:00): 22 backend (8 pure-function + 10 regex + 4 REST) + 18 frontend (16 card component + 2 store action) = 40 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Model Promotion Readiness Check via Chat** — Track B perpetual. Analysts can ask "promotion readiness check", "is my model ready to promote to production?", "pre-deployment checklist", "deployment gate", "go/no-go assessment", "production readiness check", "do all checks pass?", "run a readiness check", or "ready to promote?" (9 NL variants in `_PROMOTION_READINESS_PATTERNS`) and receive a `PromotionReadinessCard` with a structured go/no-go checklist synthesizing all available quality signals. Closes the "is this model good enough to deploy?" gap — distinct from `ModelQualityScoreCard` (single quality score) and `_READINESS_PATTERNS` (broad intent detection). `compute_promotion_readiness(metrics, algorithm, problem_type, n_rows, n_features)` pure function in `core/advisor.py` evaluates 6 independent gates: **Model Quality** (primary metric vs tiered thresholds: R² ≥0.80 pass, ≥0.65 warn; accuracy ≥0.85 pass, ≥0.70 warn), **CV Stability** (cv_std: <0.05 pass, <0.10 warn, ≥0.10 fail), **Overfitting Risk** (train-CV gap: <0.10 pass, <0.20 warn, ≥0.20 fail), **Calibration** (Brier score, classification only: <0.15 pass, <0.25 warn, ≥0.25 fail; skipped if not available), **Data Volume** (n_rows: ≥200 pass, ≥50 warn, <50 fail), **Sample-to-Feature Ratio** (rows/features: ≥20 pass, ≥10 warn, <10 fail). Overall verdict: `ready` (0 fail + 0 warn), `ready_with_warnings` (0 fail + ≥1 warn), `not_ready` (≥1 fail). Returns per-gate status + blocking_issues list + warnings list + summary. `GET /api/models/{run_id}/promotion-readiness` REST endpoint. Chat handler uses stored metrics plus CSV-loaded n_rows/n_features for accurate assessment. `PromotionReadinessCard` (emerald=ready / amber=ready_with_warnings / rose=not_ready): verdict badge (🚀/⚠️/🛑 icon), passed/warn/fail count row, per-gate `GateRow` with ✓/⚠/✗ icon + Pass/Warning/Fail badge + detail text + italic recommendation, blocking issues callout with `role="alert"`, "Deploy my model →" CTA button (visible when ready/ready_with_warnings and `onActionClick` provided). Full TypeScript wiring: `PromotionReadinessGate` + `PromotionReadinessResult`; `api.models.promotionReadiness()`; `attachPromotionReadinessToLastMessage` Zustand action. Directly closes the "am I confident enough to click Deploy?" analyst gap — the natural question after running quality, calibration, overfitting, and sample-size checks separately.
      *Day 84 (20:00): 42 backend (28 pure-function + 4 false-positive guards + 10 regex + 3 REST endpoint integration) + 26 frontend (23 card component + 3 store action) = 68 new tests. Backend lint: clean. Frontend build + TypeScript + lint: clean.*

- [x] **Deployment Prediction Distribution Comparison via Chat** — Track D perpetual. Analysts can ask "is my new deployment predicting higher values?", "compare my deployment prediction distributions", "deployment version prediction comparison", "how different are my deployment predictions", "has my retrained model changed its predictions", "old vs new deployment predictions", "production prediction distribution comparison across versions", or "deployment prediction comparison" (8 NL variants in `_DEPLOY_PRED_DIST_COMPARE_PATTERNS`) and receive a `DeploymentPredictionDistributionCard` comparing production prediction distributions across the two most recent active deployments for the project. Closes the "did my retrained model produce higher/lower values in production?" analyst gap — distinct from `DeploymentVersionComparisonCard` (training metrics) and `PredictionValueTrendCard` (trend within one deployment). `compute_deployment_prediction_comparison(baseline_logs, current_logs, problem_type)` pure function in `core/analyzer.py`: for regression, computes mean/median/std/min/max/p25/p75/n for each deployment, derives `mean_shift_pct`, verdict (current_higher / current_lower / similar); for classification, builds class frequency distributions, identifies largest shift, verdict (distribution_shifted / similar). Raises ValueError when < 5 logs in either list. `GET /api/deploy/{id}/prediction-distribution-comparison?vs=<baseline_id>` REST endpoint in `api/deploy.py`: loads up to 200 PredictionLogs from each deployment, returns no_data when either has < 5; enriches result with algorithm names and target column. Chat handler auto-selects the most recent OTHER active deployment for the project; if none exists, returns guidance; emits `{type:"deploy_pred_dist_compare"}` SSE event with system_prompt injection (verdict + summary). `DeploymentPredictionDistributionCard` (emerald=current_higher / rose=current_lower / amber=distribution_shifted / sky=similar / slate=no_data): verdict badge, problem_type badge, n_baseline/n_current badge; algorithm labels; regression: mean-shift highlight row + dual stat grids (8 stats each for baseline and current); classification: class distribution comparison table (class, previous%, current%, shift_pct ± pp); summary paragraph; sr-only figcaption. `DeploymentPredictionDistStats` + `DeploymentPredictionClassShift` + `DeploymentPredictionComparisonResult` TypeScript interfaces; `deploy_pred_dist_compare?` on `ChatMessage`; `attachDeployPredDistCompareToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 84 (04:00): 35 backend (22 pure-function + 10 regex + 3 REST) + 19 frontend (17 card + 2 store action) = 54 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Low-Activity Alert via Chat** — Track D perpetual. Analysts say "alert me if my model gets fewer than 10 predictions per day", "set up a low-activity alert", "notify me when my endpoint goes quiet", "alert me when daily predictions drop below 5", "configure a minimum prediction count alert", "add a low activity alert", "enable low-activity alert", or "alert me when my model stops receiving enough calls" (8 NL variants in `_LOW_ACTIVITY_ALERT_PATTERNS`) and receive a `LowActivityAlertCard` showing their configured floor. A `low_activity` webhook fires (max once per 24 hours) when the rolling daily prediction count falls below `threshold_per_day` — detecting broken upstream integrations that stop calling the model silently. Closes the "I don't know my integration broke until users complain" ops gap — distinct from `quota_alert` (fires approaching max quota) and `sla_alert` (fires on latency). `EVENT_LOW_ACTIVITY = "low_activity"` constant added to `core/webhook.py` `ALL_EVENTS`. `Deployment.low_activity_threshold_per_day: Optional[int]` + `low_activity_alert_last_fired_at: Optional[datetime]` fields on model. `_check_and_fire_low_activity_alert(deployment_id)` in `api/deploy.py`: queries today's PredictionLog count (midnight UTC cutoff), skips when count ≥ threshold or cooldown (24h) active, stamps `low_activity_alert_last_fired_at`, dispatches with payload `{deployment_id, daily_prediction_count, threshold_per_day, message}`. Wired into `_scheduler_loop()` (runs every 60s alongside batch jobs + weekly digests) by querying all active deployments with `low_activity_threshold_per_day IS NOT NULL`. REST: `PUT /api/deploy/{id}/low-activity-alert` (enable with threshold ≥ 1, null to disable) + `GET /api/deploy/{id}/low-activity-alert-status`. Chat handler: `_DISABLE_LOW_ACTIVITY_ALERT_RE` / `_STATUS_LOW_ACTIVITY_ALERT_RE` / `_LOW_ACTIVITY_THRESHOLD_RE` (extracts numeric threshold from message); enable intent persists threshold; disable intent clears fields; emits `{type:"low_activity_alert_config"}` SSE event. `LowActivityAlertCard` (sky border, 🔕 icon): Enabled/Disabled badge; threshold description (📉 fires when below N predictions/day); cooldown note; last-fired timestamp if any; "No alerts fired yet" placeholder; footer help text. `LowActivityAlertConfig` TypeScript interface; `low_activity_alert_config?` on `ChatMessage`; `attachLowActivityAlertConfigToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 88 (20:00): 20 backend (3 constants + 3 model fields + 6 check-function unit tests + 4 REST integration + 4 regex/NL) + 16 frontend (13 card component + 3 store action) = 36 new tests. Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction High-Activity Burst Alert via Chat** — Track D perpetual. Analysts say "alert me when my model gets more than 100 predictions per hour", "set up a high-activity alert", "configure a traffic burst alert", "notify me when my endpoint gets too many calls", "alert me when hourly predictions exceed 500", "add a burst alert", "enable high activity alert", or "alert me when my model gets flooded" (8 NL variants in `_HIGH_ACTIVITY_BURST_PATTERNS`) and receive a `HighActivityBurstCard` showing their configured ceiling. A `high_activity_burst` webhook fires (max once per hour) when the rolling 60-minute prediction count EXCEEDS `threshold_per_hour` — detecting runaway loops, API key abuse, or unexpected demand spikes. Symmetric counterpart to the Low-Activity Alert (which fires when daily count DROPS below a floor). Distinct from rate limiting (which blocks requests); this only notifies after the fact. `EVENT_HIGH_ACTIVITY_BURST = "high_activity_burst"` constant added to `core/webhook.py` `ALL_EVENTS`. `Deployment.high_activity_threshold_per_hour: Optional[int]` + `high_activity_burst_last_fired_at: Optional[datetime]` fields on model; inline migrations in `db.py` (also adds the missing `low_activity_threshold_per_day` + `low_activity_alert_last_fired_at` migrations). `_check_and_fire_high_activity_burst(deployment_id)` in `api/deploy.py`: queries PredictionLog count for the last 60 minutes, skips when count ≤ threshold or cooldown (1h) active, stamps `high_activity_burst_last_fired_at`, dispatches with payload `{deployment_id, hourly_prediction_count, threshold_per_hour, message}`. Wired into `_scheduler_loop()` alongside the low-activity check. REST: `PUT /api/deploy/{id}/high-activity-burst-alert` (enable with threshold ≥ 1, null to disable) + `GET /api/deploy/{id}/high-activity-burst-alert-status`. Chat handler: `_DISABLE_HIGH_ACTIVITY_BURST_RE` / `_STATUS_HIGH_ACTIVITY_BURST_RE` / `_HIGH_ACTIVITY_THRESHOLD_RE` (extracts numeric per-hour threshold); enable intent persists threshold; disable intent clears fields; emits `{type:"high_activity_burst_config"}` SSE event. `HighActivityBurstCard` (amber border, 📈 icon): Enabled/Disabled badge; threshold description (🔔 fires when above N predictions/hour); cooldown note; last-fired timestamp if any; "No alerts fired yet" placeholder; footer help text. `HighActivityBurstConfig` TypeScript interface; `high_activity_burst_config?` on `ChatMessage`; `attachHighActivityBurstConfigToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 89 (04:00): 21 backend (3 constants + 3 model fields + 7 check-function unit tests + 6 REST integration + 4 regex/NL) + 16 frontend (13 card component + 3 store action) = 37 new tests. Baseline: 6104 backend / 3444 frontend → **6125 backend / 3460 frontend** (+21 / +16). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Latency Alert via Chat** — Track D perpetual. Analysts say "alert me if predictions take more than 500ms", "set up a latency alert", "configure a response time alert", "notify me when my model gets too slow", "alert me when p95 latency exceeds 1000ms", "add a latency alert", "enable slow prediction alert", or "warn me when latency goes above 200ms" (8 NL variants in `_LATENCY_ALERT_PATTERNS`) and receive a `LatencyAlertCard` showing their configured p95 ceiling. A `latency_alert` webhook fires (max once per hour) when the rolling p95 response latency over the last 100 `PredictionLog.response_ms` values EXCEEDS `latency_alert_threshold_ms` — catching model slowdowns before users notice. Closes the "my model started taking 2 seconds per prediction and I didn't know" ops gap — distinct from `SlaCard` (stats display only) and `sla_exceeded` (existing fixed-threshold alert). `EVENT_LATENCY_ALERT = "latency_alert"` constant added to `core/webhook.py` `ALL_EVENTS`. `Deployment.latency_alert_threshold_ms: Optional[int]` + `latency_alert_last_fired_at: Optional[datetime]` fields on model; inline migrations in `db.py`. `_check_and_fire_latency_alert(deployment_id)` in `api/deploy.py`: queries last 100 `PredictionLog` entries with non-null `response_ms`, computes p95 via `_percentile()`, skips when p95 ≤ threshold or cooldown (1h) active or no latency data available, stamps `latency_alert_last_fired_at`, dispatches with payload `{deployment_id, p95_latency_ms, threshold_ms, sample_size, message}`. Wired into `_scheduler_loop()` alongside the low- and high-activity checks. REST: `PUT /api/deploy/{id}/latency-alert` (enable with threshold_ms ≥ 1, null to disable) + `GET /api/deploy/{id}/latency-alert-status`. Chat handler: `_DISABLE_LATENCY_ALERT_RE` / `_STATUS_LATENCY_ALERT_RE` / `_LATENCY_THRESHOLD_MS_RE` (extracts numeric ms from message); enable intent persists threshold; disable intent clears fields; emits `{type:"latency_alert_config"}` SSE event. `LatencyAlertCard` (orange border, ⏱ icon): Enabled/Disabled badge; threshold description (🔔 fires when p95 over last 100 requests exceeds N ms); cooldown note; last-fired timestamp if any; "No alerts fired yet" placeholder; footer help text. `LatencyAlertConfig` TypeScript interface; `latency_alert_config?` on `ChatMessage`; `attachLatencyAlertConfigToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 89 (12:00): 21 backend (2 constants + 1 model fields + 7 check-function unit tests + 6 REST integration + 5 regex/NL) + 16 frontend (13 card component + 3 store action) = 37 new tests. Baseline: 6125 backend / 3460 frontend → **6146 backend / 3476 frontend** (+21 / +16). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Deployment Accuracy-Triggered Auto-Rollback via Chat** — Track D perpetual. Analysts say "auto-rollback", "enable auto-rollback", "configure auto rollback", "roll back if accuracy drops below 80%", "automatically revert when performance degrades", "rollback if accuracy falls below 75%", "accuracy threshold for rollback", "disable auto rollback", or "status of auto-rollback" (8 NL variants in `_AUTO_ROLLBACK_PATTERNS`) and receive an `AutoRollbackCard` showing their configured accuracy threshold. When enabled, the scheduler checks every 60 seconds: if feedback accuracy (computed from the last 50 `FeedbackRecord.is_correct` values) drops below the threshold, the deployment automatically reverts to the previous `DeploymentVersion` and fires a `rollback_triggered` webhook. Closes the "my model accuracy degraded in production and I didn't catch it in time" ops gap — distinct from manual rollback (user-initiated) and accuracy alerts (notify-only). Requires ≥10 labeled feedback points; enforces a 24-hour cooldown to prevent rollback loops; requires ≥2 deployment versions. `EVENT_ROLLBACK_TRIGGERED = "rollback_triggered"` constant added to `core/webhook.py` `ALL_EVENTS`. `Deployment` model gets 3 new fields: `auto_rollback_enabled: bool = False`, `auto_rollback_accuracy_threshold: Optional[float]` (stored 0.0–1.0), `auto_rollback_triggered_at: Optional[datetime]`; inline migrations in `db.py`. `_check_and_fire_accuracy_rollback(deployment_id)` in `api/deploy.py`: checks enabled + threshold, 24h cooldown gate, requires ≥2 `DeploymentVersion` entries and ≥10 `FeedbackRecord` with `is_correct`; computes accuracy from last 50 feedback records; if accuracy < threshold, executes rollback (same logic as `rollback_deployment()`), stamps `auto_rollback_triggered_at`, dispatches webhook with payload `{accuracy_pct, threshold_pct, n_feedback, rolled_back_to_version, new_version_number, message}`; entirely best-effort (`except Exception: pass`). Wired into `_scheduler_loop()`: queries all active deployments with `auto_rollback_enabled=True` and calls the check function. REST: `PUT /api/deploy/{id}/auto-rollback` (enable with 0–100 accuracy threshold percentage, disable with `enabled=false`) + `GET /api/deploy/{id}/auto-rollback-status`. Chat handler: `_DISABLE_AUTO_ROLLBACK_RE` / `_STATUS_AUTO_ROLLBACK_RE` / `_AUTO_ROLLBACK_THRESHOLD_RE` sub-patterns; handler enables/disables/shows status; emits `{type:"auto_rollback_config"}` SSE event. `AutoRollbackCard` (emerald border=enabled+never triggered / amber=triggered / slate=disabled, 🔄 icon): Enabled/Disabled badge; threshold tile; how-it-works info block; last-triggered timestamp or "No auto-rollbacks triggered yet"; footer note distinguishing from manual rollback. `AutoRollbackConfig` TypeScript interface; `auto_rollback_config?` on `ChatMessage`; `attachAutoRollbackConfigToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 89 (20:00): 22 backend (2 constants + 3 model fields + 8 check-function unit tests + 4 REST integration + 4 regex/NL) + 18 frontend (15 card component + 3 store action) = 40 new tests. Baseline: 6146 backend / 3476 frontend → **6168 backend / 3494 frontend** (+22 / +18). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Value Trend Alert via Chat** — Track D perpetual. Analysts say "alert me when my average predictions drop by more than 15%", "set up a prediction value alert", "configure output trend alert", "notify me if my mean prediction shifts", "alert me if revenue prediction drops more than 20%", "prediction value trend alert", "enable a value trend alert", or "warn me when average output changes more than 10%" (8 NL variants in `_PRED_VALUE_ALERT_PATTERNS`) and receive a `PredValueAlertCard` showing their configured percentage threshold. Closes the "my model predictions silently drifted and I didn't notice until users complained" ops gap — distinct from `PredictionValueTrendCard` (on-demand historical snapshot), `accuracy_alert` (requires labeled feedback), and `latency_alert` (tracks speed, not output values). When enabled, the scheduler checks every 60 seconds: loads the last 100 `PredictionLog` entries, splits them chronologically into early (older 50) and recent (newer 50) halves, computes mean for each half using `prediction_numeric` (regression) or falls back to `confidence` (classification), computes `change_pct = (recent_mean - early_mean) / abs(early_mean) * 100`, fires if `abs(change_pct) > pred_value_alert_pct`. Requires ≥20 samples; enforces 24-hour cooldown. `EVENT_PRED_VALUE_TREND_ALERT = "pred_value_trend_alert"` constant added to `core/webhook.py` `ALL_EVENTS`. `Deployment` model gets 3 new fields: `pred_value_alert_enabled: bool = False`, `pred_value_alert_pct: Optional[float]`, `pred_value_alert_last_fired_at: Optional[datetime]`; inline migrations in `db.py`. `_check_and_fire_pred_value_trend_alert(deployment_id)` in `api/deploy.py`: entirely best-effort (`except Exception: pass`); dispatches webhook with payload `{early_mean, recent_mean, change_pct, threshold_pct, sample_size, message}`. Wired into `_scheduler_loop()`. REST: `PUT /api/deploy/{id}/pred-value-alert` (enable with `alert_pct` 0–100 exclusive, disable with `enabled=false`) + `GET /api/deploy/{id}/pred-value-alert-status`. Chat handler: `_DISABLE_PRED_VALUE_ALERT_RE` / `_STATUS_PRED_VALUE_ALERT_RE` / `_PRED_VALUE_ALERT_PCT_RE` sub-patterns; enable intent persists pct; disable intent clears fields; emits `{type:"pred_value_alert_config"}` SSE event. `PredValueAlertCard` (emerald border=enabled+never fired / amber=fired / slate=disabled, 📈 icon): Enabled/Disabled badge; shift threshold tile with orange badge; how-it-works info block; last-fired timestamp or "No alerts fired yet"; footer note. `PredValueAlertConfig` TypeScript interface; `pred_value_alert_config?` on `ChatMessage`; `attachPredValueAlertConfigToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 90 (04:00): 23 backend (2 constants + 3 model fields + 8 check-function unit tests + 6 REST integration + 4 regex/NL) + 18 frontend (15 card component + 3 store action) = 41 new tests. Baseline: 6168 backend / 3494 frontend → **6191 backend / 3512 frontend** (+23 / +18). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Confidence Heatmap by Feature Value via Chat** — Track D perpetual. Analysts say "confidence heatmap", "which combinations make my model least confident?", "where is my model uncertain?", "show me a confidence heatmap for age and region", "which input pairs produce low confidence?", "confidence heat map", "model uncertainty grid", or "where does my model struggle across feature combinations?" (8 NL variants in `_CONF_HEATMAP_PATTERNS`) and receive a `ConfidenceHeatmapCard` — a 2D CSS-grid visualization showing average model confidence across all combinations of two input feature values. Closes the "I know my model has weak spots but I don't know which inputs trigger them" gap — distinct from `SegmentConfidenceTrendCard` (time-series per segment) and `ConfidenceDistributionCard` (overall histogram, training-time). `compute_confidence_heatmap(logs, feat_x, feat_y)` pure function in `core/analyzer.py`: bins numeric features into 2–6 equal-width ranges, keeps categorical as-is (up to 8 values), computes avg confidence per cell, marks cells with avg < 65% as low-confidence zones, verdicts: `gaps_found` / `uniform_high` / `uniform_moderate` / `insufficient_data` / `no_data`; requires ≥5 valid logs and ≥3 samples per cell. `GET /api/deploy/{id}/confidence-heatmap` REST endpoint (auto-detects feature pair; prefers categorical + numeric). `_detect_heatmap_features()` helper + `_CONF_HEATMAP_FEAT_RE` sub-pattern for named feature extraction. Chat handler auto-detects feature pair from active deployment's PredictionLog history; emits `{type:"conf_heatmap"}` SSE event. `ConfidenceHeatmapCard`: 4-color CSS grid (rose < 65% / amber 65–75% / lime 75–85% / emerald ≥ 85%), cell tooltips with value + count, color legend, rose alert callout for each low-confidence zone showing exact feature values + sample counts. `ConfidenceHeatmapCell` + `ConfidenceHeatmapResult` TypeScript interfaces; `conf_heatmap?` on `ChatMessage`; `attachConfHeatmapToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 90 (12:00): 35 backend (18 pure-function + 8 regex + 5 REST endpoint + 4 false-positive guards) + 18 frontend (15 card component + 3 store action) = 53 new tests. Baseline: 6191 backend / 3512 frontend → **6226 backend / 3530 frontend** (+35 / +18). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Feature Impact Sweep via Chat** — Track D perpetual. Analysts say "which feature values produce the most extreme predictions?", "what inputs maximize the prediction?", "feature impact sweep", "sweep all features", "what combination gives the highest prediction?", "which inputs have the biggest prediction impact?", "most impactful feature values", "extreme prediction analysis", or "which feature moves the prediction the most?" (9 NL variants in `_FEATURE_SWEEP_PATTERNS`). Direction auto-detected from message: minimize if analyst says "minimize/lowest/worst-case", else maximize. Returns a `FeatureSweepCard` ranking every model feature by how much its value range moves predictions — the closest thing to a one-click "how do I maximize my outcome?" answer. Closes the "I have a deployed model but don't know which input to tweak for best results" analyst gap — distinct from `SensitivityCard` (single analyst-specified feature) and `PartialDependencePlot` (statistical PDP, not optimal-config). `run_feature_sweep(pipeline_path, model_path, direction, n_steps)` pure function in `core/deployer.py`: builds base input dict (numeric features at training mean, categorical at `le.classes_[0]`); sweeps each numeric feature across mean±2σ in `n_steps` steps; sweeps each categorical feature across all known classes; records `best_value`, `worst_value`, `best_prediction`, `worst_prediction`, `delta`; sorts by `delta` descending; adds `rank` starting at 1; computes `optimal_config` (all features at best values) and `optimal_prediction`. For classification, uses `proba.max()` as the score (confidence of winning class). `GET /api/deploy/{id}/feature-sweep` REST endpoint: `direction` (maximize/minimize, default maximize), `n_steps` (5–20, default 10). Chat handler: detects deployment context, calls `run_feature_sweep()`, injects top feature + optimal prediction into system prompt. `FeatureSweepCard` (teal border, 🔭 icon): Maximize/Minimize badge, n-features badge, target column badge; Optimal Configuration section with large prediction value + feature value badges (up to 8, +N more); Feature Impact Ranking: top 10 features with rank badge, feature name, type badge, emerald=best / rose=worst values with predictions, proportional delta bar. `FeatureSweepEntry` + `FeatureSweepResult` TypeScript interfaces; `feature_sweep?` on `ChatMessage`; `attachFeatureSweepToLastMessage` Zustand action; SSE handlers + card render in `project/[id]/page.tsx`.
      *Day 90 (20:00): 28 backend (10 regex/direction + 12 pure-function + 6 REST endpoint) + 19 frontend (16 card component + 3 store action) = 47 new tests. Baseline: 6226 backend / 3530 frontend → **6254 backend / 3549 frontend** (+28 / +19). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Saved Scenario Comparison via Chat** — Track D perpetual. Analysts save named what-if configurations and compare them side by side. Three chat intents: SAVE (`_SAVE_SCENARIO_PATTERNS` — 8 NL variants: "save this as Q2 Base", "remember this scenario as High Risk", "bookmark this prediction as Control", "save discount=0.1 quantity=100 as Q2 Optimistic", "store this config as Baseline", "add this to my scenarios as Control", "save region=West as West Case", "name this as Pessimistic"); VIEW/COMPARE (`_VIEW_SCENARIOS_PATTERNS` — 8 NL variants: "show my saved scenarios", "compare my scenarios", "scenario comparison", "scenario library", "list my what-if scenarios", "compare saved predictions", "scenario overview", "review my saved scenarios"); DELETE (`_DELETE_SCENARIO_PATTERNS` — 4 NL variants: "delete scenario Base Case", "remove scenario Q2 Base", "clear all scenarios", "delete saved scenarios"). `_parse_save_scenario_request(message)` helper: extracts scenario name (after "as") and feature overrides (`key=value` pairs before "as") using regex; returns `(name, overrides_dict)`. `SavedScenario` SQLModel table in `models/saved_scenario.py`: `id`, `deployment_id`, `name`, `inputs` (JSON), `prediction_value`, `prediction_numeric`, `confidence`, `created_at`. 4 REST endpoints in `api/deploy.py`: `GET /api/deploy/{id}/scenarios` (list + comparison summary), `POST /api/deploy/{id}/scenarios` (save with name validation), `DELETE /api/deploy/{id}/scenarios/{sc_id}` (delete one), `DELETE /api/deploy/{id}/scenarios` (clear all). `compute_scenario_comparison(scenarios_data, problem_type)` pure function in `core/deployer.py`: regression → sorts by `prediction_numeric`, identifies `best_scenario` (highest) + `worst_scenario` (lowest), computes `spread` (max−min); classification → counts class distribution, identifies mode class; empty list → `summary = "No saved scenarios yet."`. Chat SAVE handler: parses message with `_parse_save_scenario_request`, applies override dict to training means, calls `predict_single()`, persists `SavedScenario` to DB, loads all scenarios and calls `compute_scenario_comparison()` for the SSE event; VIEW handler: loads all scenarios and calls same pure function; DELETE handler: by name (or clear all). All three emit `{type:"saved_scenarios"}` SSE event. `SavedScenariosCard` (sky border, 📋 icon): n-scenarios badge, target-column badge, Regression/Classification badge; empty state with example prompt; per-scenario rows: name, Best/Worst badges (only when 2+ scenarios), prediction value (right-aligned mono), input chips (up to 4 `key=value`), regression bar proportional to min/max range (emerald=best / rose=worst / sky=other), classification confidence%; Range callout for regression spread (2+ scenarios); summary paragraph; sr-only figcaption. Full TypeScript wiring: `SavedScenarioEntry`, `SavedScenariosResult` interfaces in `lib/types.ts`; `saved_scenarios?` on `ChatMessage`; `attachSavedScenariosToLastMessage` Zustand action; SSE handlers + card render in both EventSource branches of `page.tsx`. Closes the "I want to name and compare multiple what-if configurations side by side" analyst gap — distinct from `MultiPredictionCard` (batch row predictions with no persistence) and `BulkScenarioComparison` (override-based tabular compare without naming/persistence).
      *Day 91 (12:00): 30 backend (5 save-pattern + 5 view-pattern + 4 delete-pattern + 4 parse-helper + 6 pure-function + 6 REST endpoint) + 19 frontend (17 card component + 2 store action) = 49 new tests. Baseline: 6254 backend / 3549 frontend → **6284 backend / 3568 frontend** (+30 / +19). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Canary Deployment Support via Chat** — Track D perpetual. Analysts can say "start a canary with 10% traffic on the v2 model", "compare canary vs control", or "promote the canary" to route a configurable % of production predictions to an alternate model version for live A/B testing before full rollout. The canonical example: "run 15% of predictions through the new model; if it performs better for a week, roll it out." Five new `Deployment` fields: `canary_run_id`, `canary_pipeline_path`, `canary_traffic_pct`, `canary_is_active`, `canary_started_at`; `PredictionLog.served_by_canary` bool flag (inline DB migrations). `compute_canary_comparison()` pure function in `core/deployer.py`: splits logs by `served_by_canary`, computes per-group stats (count, avg prediction, avg confidence); verdicts: `canary_better` / `canary_worse` / `similar` / `insufficient_data`. 4 REST endpoints in `api/deploy.py`: `POST /api/deploy/{id}/canary/start` (run_id + pipeline_path + traffic_pct), `POST /canary/cancel`, `POST /canary/promote`, `GET /canary/status`; canary routing logic integrated into `make_prediction()`. Chat: `_CANARY_PATTERNS` (10 NL variants) + sub-patterns for promote/cancel/traffic/version extraction; handler enables/disables/reads config + performs start/promote/cancel from chat; emits `{type:"canary_status"}` SSE. `CanaryCard` (amber border when active, slate when inactive): side-by-side control vs canary grid; live comparison stats (delta %, per-group counts/averages); available versions picker; control/cancel/promote commands in footer. Full TypeScript wiring: `CanaryVersionInfo`, `CanaryComparison`, `CanaryStatusResult` interfaces; `canary_status?` on `ChatMessage`; API methods; `attachCanaryStatusToLastMessage` action; SSE handlers in both EventSource branches. Distinct from bulk A/B testing (`BulkScenarioComparison`, offline) and feature impact (`FeatureSweepCard`, what-if).
      *Day 92 (04:00): 30 backend (7 pattern + 5 sub-pattern + 10 pure-function + 8 REST endpoint) + 35 frontend (29 card component + 6 store) = 65 new tests. Baseline: 6289 backend / 3568 frontend → **6319 backend / 3603 frontend** (+30 / +35). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Deployment Health Scorecard via Chat** — Track D perpetual. Analysts ask "show deployment health scorecard", "is my deployment healthy?", "deployment health overview", "give me a health grade", "how is my deployment doing", "health scorecard", or "what is the overall deployment health" (8 NL variants in `_DEPLOY_HEALTH_SCORECARD_PATTERNS`) and receive a `DeploymentHealthScorecardCard` with an overall A–F health grade aggregating 5 operational signals. `compute_deployment_health_scorecard(logs, feedback_records, created_at, problem_type, canary_is_active, canary_traffic_pct)` pure function in `core/analyzer.py`: (1) Latency — p95 `response_ms` from last 100 logs (green <500ms, amber 500–2000ms, red >2000ms, gray <5 samples); (2) Confidence — avg confidence from last 50 logs (green ≥75%, amber 60–75%, red <60%, gray no data); (3) Activity — predictions in last 7 days (green ≥10, amber 1–9, red 0, gray deployment <24h old); (4) Accuracy — feedback accuracy from `FeedbackRecord.is_correct` (green ≥85%, amber 55–85%, red <55%, gray <10 records); (5) Model Age — days since `created_at` (green <14d, amber 30–90d, red >90d). Weighted composite: latency 25%, confidence 20%, activity 25%, accuracy 20%, age 10% (gray signals excluded and weight redistributed). A ≥85, B 70–84, C 55–69, D 40–54, F <40. `GET /api/deploy/{id}/health-scorecard` REST endpoint. Chat handler emits `{type:"deployment_health_scorecard"}` SSE. `DeploymentHealthScorecardCard` (border color by health: emerald=healthy, amber=watching, orange=warning, rose=critical): grade badge (A–F colored), score/100 chip, health label, warning/critical counts, algorithm→target chip, 5 signal rows (icon + label + status dot + value label + finding text), canary banner when active, summary paragraph. Full TypeScript wiring: `DeploymentHealthSignal`, `DeploymentHealthCanaryInfo`, `DeploymentHealthScorecardResult` interfaces; `deployment_health_scorecard?` on `ChatMessage`; `api.deploy.healthScorecard()` client method; `attachDeploymentHealthScorecardToLastMessage` Zustand action; SSE handlers in both EventSource branches; card render in `page.tsx`. Distinct from `MonitoringDigestCard` (drift/anomaly/retraining signals) and `_HEALTH_PATTERNS` model health check (retraining guidance).
      *Day 92 (12:00): 38 backend (20 pure-function + 13 regex + 5 REST endpoint) + 24 frontend (22 card component + 2 store action) = 62 new tests. Baseline: 6319 backend / 3603 frontend → **6357 backend / 3627 frontend** (+38 / +24). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Production Input Distribution Drift Alert** — Track D perpetual. Analysts say "enable input distribution drift alerts", "alert me when live inputs diverge from training", "notify me when prediction inputs drift" (8 NL variants in `_INPUT_DIST_DRIFT_ALERT_PATTERNS`) to set up a proactive webhook that fires when live prediction inputs diverge from the training distribution. `EVENT_INPUT_DIST_DRIFT = "input_dist_drift"` webhook constant. Three new `Deployment` fields: `input_dist_drift_alert_enabled` (bool, default False), `input_dist_drift_severity_threshold` (str "medium"|"high", default "medium"), `input_dist_drift_alert_last_fired_at` (datetime). `_check_and_fire_input_dist_drift_alert(deployment_id)` in `api/deploy.py`: loads last 200 PredictionLogs, parses `input_features` JSON, calls existing `compute_covariate_drift_alert()` pure function from `core/analyzer.py`, compares severity rank against threshold ("medium"=≥15% OOR, "high"=≥30% OOR), fires webhook with 24-hour cooldown, payload includes `severity`, `severity_label`, `alert_count`, `sample_count`, `top_drifted_features`, `message`. REST: `PUT /api/deploy/{id}/input-dist-drift-alert` + `GET /api/deploy/{id}/input-dist-drift-alert-status`. Wired into `_scheduler_loop()`. Chat handler with `_DISABLE_INPUT_DIST_DRIFT_ALERT_RE`, `_STATUS_INPUT_DIST_DRIFT_ALERT_RE`, `_HIGH_THRESHOLD_INPUT_DIST_RE` (detects "high severity" phrasing). Frontend: `InputDistDriftAlertConfig` TypeScript interface; `input_dist_drift_alert_config?` on `ChatMessage`; `attachInputDistDriftAlertConfigToLastMessage` Zustand action; SSE handlers; `InputDistDriftAlertCard` (cyan border, 🌊 icon, threshold label "Medium (≥15% OOR)" vs "High (≥30% OOR)"). Distinct from on-demand `CovariateDriftAlertCard` (per-request display) and `FeatureDriftAlertCard` (fires on feature importance × drift ranking). Minimum 20 samples required before alert can fire.
      *Day 92 (20:00): 24 backend (3 constant/model + 10 unit + 7 REST endpoint + 4 regex) + 18 frontend (15 card component + 3 store) = 42 new tests. Baseline: 6357 backend / 3612 frontend → **6381 backend / 3630 frontend** (+24 / +18). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Confidence Band Chart** — Track D perpetual. Analysts ask "show me the confidence band", "prediction uncertainty over time", "how confident has my model been", "confidence band chart", "prediction stability over time", "how stable are my predictions", "prediction spread over time", or "confidence interval trend" (8 NL variants in `_CONFIDENCE_BAND_PATTERNS`) and receive a `ConfidenceBandCard` showing daily mean ± 1 std prediction values (regression) or confidence scores (classification) as a shaded band over the last N days — closing the "how stable and certain are my model's outputs over time?" analyst gap. Distinct from `ConfidenceTrendCard` (direction of confidence, no band), `PredictionValueTrendCard` (overall trend, no spread), and `CovariateDriftAlertCard` (input drift). `compute_confidence_band(logs_data, problem_type, n_days)` pure function in `core/analyzer.py`: groups PredictionLog dicts by day (ISO string truncation `str(ts)[:10]`); for regression uses `prediction_numeric`, for classification uses `confidence` (skips out-of-range values ≤0 or ≥1); computes per-day `mean`, `std`, `lower = mean − std`, `upper = mean + std`; overall CoV (std/|mean|) to determine verdict (`stable` < 5%, `moderate_spread` 5–20%, `high_spread` > 20%, `no_data` < 5 samples); returns `days[]`, `means[]`, `lowers[]`, `uppers[]`, `overall_mean`, `overall_std`, `overall_lower`, `overall_upper`, `n_samples`, `n_days`, `verdict`, `problem_type`, `value_label`, `summary`. `GET /api/deploy/{id}/confidence-band?n_days=30&n=200` REST endpoint in `api/deploy.py`: loads last 200 PredictionLogs ascending by `created_at`, calls pure function, returns result + `deployment_id`, `algorithm`, `target_column`. Chat handler guarded by `ctx["deployment"]`; loads 200 logs; injects verdict + summary into system_prompt; emits `{type:"confidence_band"}` SSE event. Two pattern fixes: `show(?:\s+me)?` to catch "show me the confidence band"; `predictions?(?:\s+over\s+time)?` to avoid requiring trailing whitespace. `ConfidenceBandCard` (sky border=stable, amber=moderate_spread, rose=high_spread, muted=no_data, 📊 icon): verdict badge + n-predictions badge + n-days badge; summary paragraph; stats grid (Average / Std Dev / Typical Range); Recharts `ComposedChart` with two `Area` (upper sky-blue fill + lower white mask) + `Line` (sky mean); sr-only figcaption. Full TypeScript wiring: `ConfidenceBandResult` interface; `confidence_band?` on `ChatMessage`; `attachConfidenceBandToLastMessage` Zustand action; `api.deploy.confidenceBand()` REST helper; SSE handlers + card render in both EventSource branches of `page.tsx`.
      *Day 93 (04:00): 36 backend (14 unit + 13 regex + 3 endpoint + async fixture chain) + 0 frontend (card, types, store, api, page.tsx) = 36 new backend tests. Baseline: 6381 backend / 3630 frontend → **6417 backend / 3630 frontend** (+36 / +0). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Degradation-Triggered Auto-Retrain** — Track D perpetual. Analysts say "retrain automatically when accuracy drops below 80%", "enable auto-retraining when performance degrades", "configure degradation-triggered retraining", "trigger retraining if accuracy falls below 75%", "retrain on degradation", or "disable degradation retraining" (8 NL variants in `_DEGRADATION_RETRAIN_PATTERNS`) to have AutoModeler automatically start a fresh training run when production feedback accuracy drops below a configured threshold. Distinct from: `AutoRollback` (reverts to a previous model version, no new training), `AutoRetrainOnUpload` (triggers on data upload, not performance), `RetrainingReadinessCard` (recommends but does not trigger). `EVENT_DEGRADATION_RETRAIN = "degradation_retrain"` webhook constant added to `ALL_EVENTS`. Three new `Deployment` fields: `degradation_retrain_enabled` (bool, default False), `degradation_retrain_accuracy_threshold` (Optional[float] 0.0–1.0), `degradation_retrain_last_triggered_at` (Optional[datetime]); inline DB migrations added. `_check_and_trigger_degradation_retrain(deployment_id)` in `api/deploy.py`: requires ≥10 feedback records, computes live accuracy from `FeedbackRecord` table, skips if accuracy ≥ threshold or within 24-hour cooldown, loads ModelRun context (project_id, algorithm, feature_set, dataset path), creates new `ModelRun`, launches `_train_in_background()` in daemon thread, dispatches `degradation_retrain` webhook with payload (accuracy_pct, threshold_pct, n_feedback, new_run_id, algorithm, message). REST: `PUT /api/deploy/{id}/degradation-retrain` (enable/disable + threshold validation 0–100%), `GET /api/deploy/{id}/degradation-retrain-status`. Wired into `_scheduler_loop()` to run every 60 seconds on enabled deployments. Chat handler with `_DISABLE_DEGRADATION_RETRAIN_RE`, `_STATUS_DEGRADATION_RETRAIN_RE`, `_DEGRADATION_RETRAIN_THRESHOLD_RE` for threshold extraction; emits `{type:"degradation_retrain_config"}` SSE event. Frontend: `DegradationRetrainConfig` TypeScript interface; `degradation_retrain_config?` on `ChatMessage`; `attachDegradationRetrainConfigToLastMessage` Zustand action; `api.deploy.setDegradationRetrain()` + `api.deploy.getDegradationRetrainStatus()` client methods; `DegradationRetrainCard` (violet border=enabled+never triggered / amber=triggered / slate=disabled, 🔁 icon): enabled/disabled badge, summary, threshold tile with accuracy floor badge (when enabled), how-it-works info block with cooldown/min-feedback/webhook details, last-triggered timestamp or "No auto-retraining triggered yet", footer note distinguishing from auto-rollback.
      *Day 93 (12:00): 22 backend (2 constants/model + 8 check-function unit + 6 REST integration + 4 regex/NL + 2 mock fix) + 18 frontend (15 card component + 3 store action) = 40 new tests. Baseline: 6417 backend / 3630 frontend → **6439 backend / 3648 frontend** (+22 / +18). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Model Retraining Completion Notification** — Track D perpetual. Analysts ask "notify me when retraining is done", "retraining complete notification", "tell me when auto-retraining finishes", "retraining completion webhook", "how do I get notified when retraining is done", or "did the retraining finish" (8 NL variants in `_RETRAIN_COMPLETE_NOTIFY_PATTERNS`) to see the last completed retrain result and webhook configuration. Distinct from: `DegradationRetrainCard` (shows when retraining is triggered, not when it completes), `AutoRollback` (reverts, doesn't train). `EVENT_RETRAIN_COMPLETE = "retrain_complete"` added to `core/webhook.py` `ALL_EVENTS`. `_train_in_background()` in `api/models.py` updated with optional `deployment_id: str | None = None` kwarg; on success fires `retrain_complete` webhook with payload: `deployment_id`, `run_id`, `algorithm`, `primary_metric`, `primary_metric_value`, `training_duration_ms`, `message`. `_check_and_trigger_degradation_retrain()` in `api/deploy.py` passes `kwargs={"deployment_id": deployment_id}` to the training thread. Chat handler queries the latest completed `ModelRun` for the deployment's project + registered webhooks for `retrain_complete` event type; emits `{type:"retrain_complete_notify"}` SSE. Frontend: `RetrainCompletedRun` + `RetrainCompleteNotifyResult` TypeScript interfaces; `retrain_complete_notify?` on `ChatMessage`; `attachRetrainCompleteNotifyToLastMessage` Zustand action; SSE handlers in both EventSource branches; `RetrainCompleteNotifyCard` (emerald border=has webhook + last run / violet=has webhook but no run yet / amber=no webhook registered, 🔔 icon): enabled/disabled badge, last completed run stats grid (algorithm/training duration/primary metric/completed at), registered webhook URLs list, how-it-works info block (payload description, webhook registration instructions), footer noting distinction from `degradation_retrain` webhook.
      *Day 93 (20:00): 21 backend (2 webhook constant + 2 signature/source + 8 pattern match + 4 no-match + 3 webhook dispatch + 2 event shape) + 20 frontend (17 card component + 3 store action) = 41 new tests. Baseline: 6439 backend / 3648 frontend → **6460 backend / 3668 frontend** (+21 / +20). Backend lint: clean. Frontend build + TypeScript: clean.*

- [x] **Prediction Outcome Calibration Chart** — Track D perpetual. Analysts can ask "prediction outcome calibration", "show me the calibration chart", "show calibration from feedback", "are my predictions calibrated based on actual outcomes", "reliability diagram", "calibration chart using feedback", "prediction error histogram from feedback", or "prediction accuracy by confidence bucket" (8 NL variants in `_OUTCOME_CALIB_PATTERNS`) and receive an `OutcomeCalibrationCard` built from production `FeedbackRecord` outcomes — not training data. Closes the "is my model well-calibrated on real production inputs that have been verified?" analyst gap — distinct from `CalibrationCheckCard` (training-time Brier/ECE on validation data) and `ProductionThresholdOptimizerCard` (sweeps thresholds, not a visual chart). `compute_prediction_outcome_calibration(feedback_pairs, problem_type, n_buckets=10)` pure function in `core/analyzer.py`: dispatches to `_classification_calibration()` (reliability diagram — 10 equal-width confidence buckets 0–100%, per-bucket mean_confidence / actual_accuracy / deviation / n, ECE = weighted |confidence−accuracy|, verdicts: `well_calibrated`/`slightly_overconfident`/`slightly_underconfident`/`overconfident`/`underconfident`) or `_regression_calibration()` (error histogram — bins of actual−predicted errors, reports mean_error/median_error/std_error/mae/rmse/bins, verdicts: `unbiased`/`positive_bias`/`negative_bias`/`low_data`); returns `no_data` when < 5 samples, `not_applicable` for unknown problem types. `GET /api/deploy/{id}/outcome-calibration` REST endpoint in `api/deploy.py`: for classification loads `FeedbackRecord` entries with linked `PredictionLog.confidence`, resolves `is_correct` from stored flag or predicted vs actual label comparison; for regression loads `FeedbackRecord.actual_value` + `PredictionLog.prediction_numeric`; calls pure function; enriches result with `deployment_id`, `algorithm`, `target_col`. Chat handler guarded by `ctx["deployment"]`; injects verdict + ECE/MAE + summary into system_prompt; emits `{type:"outcome_calibration"}` SSE event. `OutcomeCalibrationCard` at `components/deploy/outcome-calibration-card.tsx` (emerald=well_calibrated/unbiased, amber=slightly_over/underconfident/low_data, rose=overconfident/underconfident/positive_bias/negative_bias, muted=no_data/not_applicable, 📐 icon): verdict badge, problem-type/sample-count/algorithm/target-col header badges, summary paragraph; classification: ECE% stat + overall accuracy stat + Recharts LineChart reliability diagram (green accuracy line, indigo confidence line, dashed gray perfect-calibration diagonal) + n_filled_buckets annotation; regression: MAE/RMSE/bias/std-dev stat grid + Recharts BarChart error histogram with red dashed mean-error reference line; sr-only figcaptions; footer distinguishing from training-time calibration. Full TypeScript wiring: `OutcomeCalibrationBucket`, `OutcomeCalibrationBin`, `OutcomeCalibrationVerdict`, `OutcomeCalibrationResult` interfaces in `lib/types.ts`; `outcome_calibration?` on `ChatMessage`; `attachOutcomeCalibrationToLastMessage` Zustand action; `api.deploy.outcomeCalibration()` REST helper; SSE handlers + card render wired in both EventSource branches of `app/project/[id]/page.tsx`.
      *Day 94 (04:00): 38 backend (2 constants + 4 no_data/not_applicable + 12 classification pure-function + 13 regression pure-function + 3 REST endpoint + 4 regex/NL match + 2 no-match guard guard) + 21 frontend (18 card component + 3 store action) = 59 new tests. Baseline: 6460 backend / 3668 frontend → **6498 backend / 3689 frontend** (+38 / +21). Backend lint: clean. Frontend build + TypeScript: clean.*
