...
class TestPredictionPerformance:
    async def _deploy_model(self, client, perf_csv):
        ...
        # Wait for training (asserts the run finished 'done'; fails loudly on a
        # 'failed' run or timeout instead of silently skipping — issue #13).
        run_id = train_resp.json()["model_run_ids"][0]
        await wait_for_run(client, pid, run_id)
