...
_DATA_PREFIX = "data"
# The live DB file and its WAL/SHM sidecars are excluded from the artifact tar —
# the DB is captured separately via the online backup API.
_DB_PREFIXES = ("automodeler.db",)
        ...
        with tarfile.open(archive, "w:gz") as tar:
            ...
            if data_root.exists():
                for path in sorted(data_root.rglob("*")):
                    if not path.is_file():
                        continue
                    # Skip the live DB / WAL / SHM — captured via online backup.
                    if path.name.startswith(_DB_PREFIXES):
                        continue
