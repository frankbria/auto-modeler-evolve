...
interface AppState {
  ...
  attachPerClassThresholdToLastMessage: (per_class_threshold: import("./types").PerClassThresholdResult) => void
...
export const useAppStore = create<AppState>((set) => ({
  ...
  attachAlertRuleToLastMessage: (alert_rule) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
        messages[messages.length - 1] = { ...last, alert_rule }
      }
      return { messages }
    }),
  attachApiKeyResultToLastMessage: (api_key_result) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachModelCardExportToLastMessage: (model_card_export) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachDashboardMetadataToLastMessage: (dashboard_metadata) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachGoalSeekToLastMessage: (goal_seek) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachCrossDeployPredictionToLastMessage: (cross_deploy_prediction) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachPredictionDeltaToLastMessage: (prediction_delta) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachFeEngineeringImpactToLastMessage: (fe_impact) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachPerClassThresholdToLastMessage: (per_class_threshold) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
        messages[messages.length - 1] = { ...last, per_class_threshold }
      }
      return { messages }
    }),
  ...
  attachErrorCorrelationToLastMessage: (error_correlation) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachMinFeatureSetToLastMessage: (min_feature_set) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachMonitoringDigestToLastMessage: (monitoring_digest) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
        messages[messages.length - 1] = { ...last, monitoring_digest }
      }
      return { messages }
    }),

  attachModelStatusReportToLastMessage: (model_status_report) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachDeployPredDistCompareToLastMessage: (deploy_pred_dist_compare) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachHighActivityBurstConfigToLastMessage: (high_activity_burst_config) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
        messages[messages.length - 1] = { ...last, high_activity_burst_config }
      }
      return { messages }
    }),

  attachLatencyAlertConfigToLastMessage: (latency_alert_config) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachSegmentDriftToLastMessage: (segment_drift) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
        messages[messages.length - 1] = { ...last, segment_drift }
      }
      return { messages }
    }),

  attachSegmentPredTrendToLastMessage: (segment_pred_trend) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
  ...
  attachOutcomeCalibrationToLastMessage: (outcome_calibration) =>
    set((state) => {
      const messages = [...state.messages]
      const last = messages[messages.length - 1]
      if (last && last.role === "assistant") {
